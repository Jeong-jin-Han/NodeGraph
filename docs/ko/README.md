...
간단히 설명해줘. 그 다음 바로 만들어줘.

**모양.** 서브노드가 **12개를 넘어갈 규모면** 계층형으로 만들어줘 — 스펙의 "Two shapes"
절을 따라. 그보다 작으면 백본 아래 한 층으로 둬도 되고, 깊이를 만들려고 중간 노드를
억지로 끼워 넣는 건 오히려 잡음이야. 적용될 때의 기준은 이거야:
...
- **층마다 다른 질문에 답해야 해**: 이게 무엇인가 / 무엇으로 이루어졌나 / 각 부분이
  어떻게 동작하나 / 그 경우·그 수치·그 예외.
- **부모만 읽어도 말이 돼야 해.** 2층까지만 보고 3층을 한 번도 안 연 사람도 거칠지만
  틀리지는 않은 이해를 가지고 나가야 해.

백본 노드는 스펙의 **kernel test**(Step 2)로 골라줘. kernel은 "저자가 기여한, 남이 하지
않은 것"이야. 4~8개를 찾고, **그 집합만으로 논문 전체가 재구성되는지** 확인해줘. 나머지는
SUPPORTING(진짜지만 표준적인 것 — 그 kernel 아래로)이나 non-kernel(노드를 아예 만들지
않음)로 분류하고. 그 다음 사슬을 써줘: 각 kernel이 어느 kernel에서 도출되고 어느 것을
가능하게 하는지. 도출되지도 않고 아무것도 가능하게 하지 않는 kernel은 분류가 틀린 거야.

모든 노드 내용은 한국어로 작성해. 단, 논문에서 쓰인 전문 용어는 임의로 의역하지
말고 그 분야에서 실제 쓰이는 정확한 학술 용어를 사용하고, 핵심 용어는 처음
등장할 때 원문 영어 표현을 괄호로 병기해(예: "다중 헤드 주의(multi-head attention)").
...
`conventions`에 적어줘 (스펙의 `conventions` 절 참고).

제목은 주제가 아니라 **틀릴 수 있는 주장**으로 써줘. "Killer Application",
"필요한 이유", "해결책", "결과", "결론"은 칸 이름이지 제목이 아니야.

다 만든 다음에 줄이기 패스를 한 번 더 돌려줘. 노드 하나를 떼어 읽어도 완결되게
쓰려는 습관이 전체를 원문보다 읽기 느리게 만들어. 노드는 문서가 아니라
슬라이드고, 완결성은 경로가 책임져.

인용의 페이지 번호(`p.N`)는 **PDF의 페이지 인덱스**(첫 장이 1)야. 논문에 인쇄된 쪽번호가
아니야 — 학회 논문집은 수백 쪽씩 어긋나고, 뷰어는 `p.N`을 그대로 PDF 인덱스로 써.
