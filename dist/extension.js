"use strict";var ne=Object.create;var B=Object.defineProperty;var oe=Object.getOwnPropertyDescriptor;var ie=Object.getOwnPropertyNames;var ae=Object.getPrototypeOf,se=Object.prototype.hasOwnProperty;var de=(t,e)=>{for(var r in e)B(t,r,{get:e[r],enumerable:!0})},W=(t,e,r,n)=>{if(e&&typeof e=="object"||typeof e=="function")for(let i of ie(e))!se.call(t,i)&&i!==r&&B(t,i,{get:()=>e[i],enumerable:!(n=oe(e,i))||n.enumerable});return t};var O=(t,e,r)=>(r=t!=null?ne(ae(t)):{},W(e||!t||!t.__esModule?B(r,"default",{value:t,enumerable:!0}):r,t)),le=t=>W(B({},"__esModule",{value:!0}),t);var Ie={};de(Ie,{activate:()=>we,deactivate:()=>Ee});module.exports=le(Ie);var h=O(require("vscode"));var l=O(require("vscode")),Z=O(require("path"));var D=O(require("vscode"));function U(t){let e=D.Uri.joinPath(t,".."),r=t.path.split("/").pop()?.replace(/\.nodegraph\.json$/,"")??"graph";return D.Uri.joinPath(e,`.${r}-imgs`)}function ce(t,e,r){let n=D.Uri.joinPath(U(e),r);return t.asWebviewUri(n).toString()}var z=/\[\[IMG:([^:\]]+)(?::[^\]]+)?\]\]/g;function j(t,e,r){let n={},i=s=>{s&&!n[s]&&(n[s]=ce(t,e,s))};for(let s of r.nodes){z.lastIndex=0;let p;for(;(p=z.exec(s.content??""))!==null;)i(p[1])}for(let s of r.canvasImages??[])i(s.filename);return n}async function q(t,e,r,n="png"){let i=U(e);try{await D.workspace.fs.createDirectory(i)}catch{}let s=`img_${Date.now()}.${n}`,p=D.Uri.joinPath(i,s);return await D.workspace.fs.writeFile(p,Buffer.from(r,"base64")),{filename:s,webviewUri:t.asWebviewUri(p).toString()}}async function V(t,e){let r=D.Uri.joinPath(U(t),e);try{await D.workspace.fs.delete(r)}catch{}}function x(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function pe(t){let e=t.trim().replace("#",""),r=e.length===3?e.split("").map(n=>n+n).join(""):e;return/^[0-9a-fA-F]{6}$/.test(r)?{r:255-parseInt(r.slice(0,2),16),g:255-parseInt(r.slice(2,4),16),b:255-parseInt(r.slice(4,6),16)}:null}var ue=t=>t.replace(/[^a-zA-Z0-9_-]/g,"_");function $(t){return/^\s*\|/.test(t)&&t.indexOf("|",1)!==-1}function X(t){return/^\s*\|[\s\-:|]+\|\s*$/.test(t)&&!/[a-zA-Z0-9]/.test(t)}function K(t){return t.replace(/^\s*\|/,"").replace(/\|\s*$/,"").split("|").map(e=>e.trim())}function he(t){if(!t)return[{type:"text",text:"",startChar:0,endChar:0}];let e=t.split(`
`),r=[],n=0,i=0,s=p=>e[p].length+(p<e.length-1?1:0);for(;n<e.length;)if($(e[n])&&n+1<e.length&&X(e[n+1])){let u=i,o=[];for(;n<e.length&&$(e[n]);)o.push(e[n]),i+=s(n),n++;o.length>=3?r.push({type:"table",headers:K(o[0]),rows:o.slice(2).map(K),startChar:u,endChar:i}):r.push({type:"text",text:o.join(`
`),startChar:u,endChar:i})}else{let u=i,o=[];for(;n<e.length&&!($(e[n])&&n+1<e.length&&X(e[n+1]));)o.push(e[n]),i+=s(n),n++;r.push({type:"text",text:o.join(`
`),startChar:u,endChar:i})}return r}function G(t){let e=t.split(`
`);for(let r=0;r+1<e.length;r++)if($(e[r])&&X(e[r+1]))return!0;return!1}function ge(t){return x(t).replace(/\\\$/g,()=>'<span class="ng-cur">$</span>')}function L(t){return ge(t).replace(/\*\*(.+?)\*\*/g,'<strong style="font-size:1.1em">$1</strong>')}function H(t,e){let r=/\[\[IMG:([^:\]]+)(?::(\d+)x(\d+))?\]\]/g,n="",i=0,s;for(;(s=r.exec(t))!==null;){s.index>i&&(n+=L(t.slice(i,s.index)));let p=s[1],u=s[2],o=s[3],d=u&&o?` width="${u}" height="${o}"`:"",c=e[p];n+=c?`<img class="ng-img${d?" ng-img-sized":""}" src="${c}"${d} alt="${x(p)}" onclick="showLightbox(this.src)" title="Click to enlarge">`:`<span class="ng-img-missing">${x(p)}</span>`,i=s.index+s[0].length}return i<t.length&&(n+=L(t.slice(i))),n}function fe(t,e){let r=t.headers.map(i=>`<th>${H(i,e)}</th>`).join(""),n=t.rows.map(i=>`<tr>${i.map(s=>`<td>${H(s,e)}</td>`).join("")}</tr>`).join("");return`<div class="ng-table-wrap"><table class="ng-table"><thead><tr>${r}</tr></thead><tbody>${n}</tbody></table></div>`}function ve(t,e,r,n,i){let s=e?.color??"#888",p=e?.shape==="rounded"?"22px":"2px",u=x(e?.label??t.template),o=Math.round(t.position.x+r),d=Math.round(t.position.y+n),c="",a=t.content??"";if(G(a)){let g=he(a);c+='<div class="ng-content">';for(let S of g)S.type==="table"?c+=fe(S,i):S.text&&(c+=`<div class="ng-seg">${H(S.text,i).replace(/\n/g,"<br>")}</div>`);c+="</div>"}else a&&(c+=`<div class="ng-content">${H(a,i).replace(/\n/g,"<br>")}</div>`);if(t.original){let g=x(t.original.title??"Original"),S=t.originalExpanded?" open":"";c+=`<details class="ng-original"${S}><summary>${g}${t.original.location?` <span class="ng-loc">${x(t.original.location)}</span>`:""}</summary>
...
<div class="ng-toggle-body">${L(g.content).replace(/\n/g,"<br>")}</div></details>`;t.links.length&&(c+=`<div class="ng-links">${t.links.map(g=>{let S=g.type==="url"?"\u{1F517}":g.type==="pdf"?"\u{1F4C4}":g.type==="obsidian"?"\u{1F7E3}":"\u2B21";return`<a class="ng-link"${g.type==="url"||g.type==="pdf"?` href="${x(g.target)}" target="_blank"`:""}>${S} ${x(g.label||g.target)}</a>`}).join("")}</div>`);let m=!!c,f=t.contentExpanded?"":' style="display:none"',A=t.children.length?` data-children="${t.children.join(",")}"`:"",C=G(a)?" ng-has-table":"",T=/\[\[IMG:[^:\]]+:(\d+)x\d+\]\]/g,w=0,N;for(;(N=T.exec(a))!==null;)w=Math.max(w,Number(N[1]));let E=w>0?G(a)?w+280:w+32:0,v=Math.max(t.nodeWidth??0,432,E),P=[v>432?`min-width:${v}px`:"",t.nodeHeight&&t.contentExpanded?`min-height:${t.nodeHeight}px`:""].filter(Boolean).join(";"),k=t.nodeHeight?` data-min-h="${t.nodeHeight}"`:"";return`<div class="ng-node${C}" id="node-${x(t.id)}"${A}${k} style="--color:${s};border-radius:${p};left:${o}px;top:${d}px${P?";"+P:""}">
  <div class="ng-header" onclick="onHeaderClick(this)" title="Click to select node">
    <span class="ng-tag" onmousedown="onNodeTagMousedown(event,this.closest('.ng-node'))" style="background:color-mix(in srgb,${s} 20%,transparent);color:${s}">${u}</span>
    ${m?`<span class="ng-title" onclick="onTitleClick(event,this)" title="Click to fold/unfold">${x(t.title)}</span>`:`<span class="ng-title">${x(t.title)}</span>`}
  </div>
  ${m?`<div class="ng-body"${f}${t.fontSize?` style="font-size:${t.fontSize}px"`:""}>${c}</div>`:""}
</div>`}function J(t,e={}){let r=1/0,n=1/0;for(let a of t.nodes)r=Math.min(r,a.position.x),n=Math.min(n,a.position.y);isFinite(r)||(r=0,n=0);let i=-r+100,s=-n+100,p=t.nodes.map(a=>ve(a,t.nodeTemplates[a.template],i,s,e)).join(`
`),u=JSON.stringify(t.nodes.map(a=>({id:a.id,lx:Math.round(a.position.x+i),ly:Math.round(a.position.y+s),children:a.children??[],template:a.template,contentExpanded:a.contentExpanded,isMain:a.template==="main_topic",nodeHeight:a.nodeHeight??null,naturalY:Math.round((a.nodeNaturalY??a.position.y)+s),title:a.title,content:a.content??"",originalTitle:a.original?.title??"",originalText:a.original?.text??"",toggles:(a.toggleItems??[]).map(m=>({id:m.id,title:m.title,content:m.content}))}))),o=JSON.stringify(t.edges.map(a=>({source:a.source,target:a.target,type:a.type,label:a.label||""}))),d=JSON.stringify(Object.fromEntries(Object.entries(t.nodeTemplates).map(([a,m])=>[a,m.label]))),c=Object.entries(t.nodeTemplates).map(([a,m])=>{let f=pe(m.color),A=f?`rgb(${f.r},${f.g},${f.b})`:"#ff3b30",C=f?`rgba(${f.r},${f.g},${f.b},0.18)`:"rgba(255,59,48,0.18)";return`::highlight(ng-hit-${ue(a)}){color:${A};background-color:${C};text-decoration:underline}`}).join(`
`);return`<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
</html>`}var me={main_topic:{label:"Main topic",color:"#4B8BBE",icon:"file-text",shape:"sharp"},method:{label:"Method",color:"#5C9E6E",icon:"cpu",shape:"sharp"},result:{label:"Result",color:"#9B59B6",icon:"bar-chart-2",shape:"sharp"},claim:{label:"Claim",color:"#E74C3C",icon:"alert-circle",shape:"sharp"},question:{label:"Question",color:"#E5A835",icon:"help-circle",shape:"rounded"},gap:{label:"Gap / Idea",color:"#1ABC9C",icon:"lightbulb",shape:"rounded"},reference:{label:"Reference",color:"#95A5A6",icon:"book-open",shape:"rounded"},memo:{label:"Memo",color:"#BDC3C7",icon:"edit-3",shape:"rounded"}};function F(t="New Graph"){let e=new Date().toISOString();return{version:"1.0.0",title:t,created:e,modified:e,nodeTemplates:me,nodes:[],edges:[],viewport:{x:0,y:0,zoom:1}}}function _(){let t="",e="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";for(let r=0;r<32;r++)t+=e.charAt(Math.floor(Math.random()*e.length));return t}var b=O(require("vscode"));var Y=class t{static{this.panels=new Map}static async openAndSearch(e,r,n,i){let s=r.toString(),p=t.panels.get(s);if(p){p.panel.reveal(b.ViewColumn.Beside,!0),p.ready?p.panel.webview.postMessage({type:"search",query:n,pageHint:i}):p.pending={query:n,pageHint:i};return}let u;try{u=await b.workspace.fs.readFile(r)}catch{b.window.showErrorMessage(`PDF\uB97C \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4: ${r.fsPath}`);return}let o=b.window.createWebviewPanel("nodegraph.pdfViewer",r.path.split("/").pop()??"PDF",{viewColumn:b.ViewColumn.Beside,preserveFocus:!1},{enableScripts:!0,retainContextWhenHidden:!0,localResourceRoots:[b.Uri.joinPath(e.extensionUri,"dist")]}),d={panel:o,ready:!1,pending:{query:n,pageHint:i}};t.panels.set(s,d),o.iconPath=b.Uri.joinPath(e.extensionUri,"resources","icon-hires.png"),o.webview.html=t._getHtml(e,o.webview);let c=Buffer.from(u).toString("base64");o.webview.onDidReceiveMessage(a=>{a.type==="ready"&&(d.ready=!0,o.webview.postMessage({type:"load",pdfData:c,query:d.pending?.query,pageHint:d.pending?.pageHint}),d.pending=null)}),o.onDidDispose(()=>{t.panels.delete(s)})}static _getHtml(e,r){let n=r.asWebviewUri(b.Uri.joinPath(e.extensionUri,"dist","pdfviewer.js")),i=r.asWebviewUri(b.Uri.joinPath(e.extensionUri,"dist","pdfviewer.css")),s=r.asWebviewUri(b.Uri.joinPath(e.extensionUri,"dist","pdf.worker.min.mjs")),p=_();return`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
</html>`}};var M=class t{constructor(e){this.context=e;this._pendingSaves=new Set}static register(e){let r=new t(e);return l.window.registerCustomEditorProvider("nodegraph.editor",r,{webviewOptions:{retainContextWhenHidden:!0}})}static{this._activeWebview=null}static postToActive(e){t._activeWebview?.postMessage(e)}async resolveCustomTextEditor(e,r,n){r.iconPath=l.Uri.joinPath(this.context.extensionUri,"resources","icon-hires.png");let i=l.Uri.joinPath(e.uri,"..");r.webview.options={enableScripts:!0,localResourceRoots:[this.context.extensionUri,i]},r.webview.html=this._getHtmlForWebview(r.webview);let s=o=>{let d=e.getText();try{let c=d.trim()===""?F():JSON.parse(d),a=j(r.webview,e.uri,c);r.webview.postMessage({type:o,data:c,imageUris:a})}catch{}},p=r.webview.onDidReceiveMessage(async o=>{if(o.type==="ready")s("load");else if(o.type==="save"){let d=e.uri.toString();this._pendingSaves.add(d);try{let c=new l.WorkspaceEdit,a=new l.Range(e.positionAt(0),e.positionAt(e.getText().length));c.replace(e.uri,a,JSON.stringify(o.data,null,2)),await l.workspace.applyEdit(c),await e.save()}finally{this._pendingSaves.delete(d)}}else if(o.type==="openLink"){let d=o.link;if(d.type==="url")l.env.openExternal(l.Uri.parse(d.target));else if(d.type==="pdf"){let c=l.Uri.joinPath(l.Uri.joinPath(e.uri,".."),d.target);l.env.openExternal(c)}else d.type==="obsidian"&&l.env.openExternal(l.Uri.parse(d.target))}else if(o.type==="searchInPdf"){let d=l.Uri.joinPath(l.Uri.joinPath(e.uri,".."),o.pdfTarget);Y.openAndSearch(this.context,d,o.query,o.pageHint)}else if(o.type==="exportHtml")try{let d=o.data,c=l.Uri.joinPath(e.uri,".."),a=Z.basename(e.uri.fsPath,".nodegraph.json"),m=l.Uri.joinPath(c,`.${a}-imgs`),f={},A=/\[\[IMG:([^:\]]+)(?::[^\]]+)?\]\]/g,C=async E=>{if(!(!E||f[E]))try{let v=l.Uri.joinPath(m,E),P=await l.workspace.fs.readFile(v),k=E.split(".").pop()?.toLowerCase()??"png",g=k==="jpg"||k==="jpeg"?"image/jpeg":k==="gif"?"image/gif":k==="webp"?"image/webp":"image/png";f[E]=`data:${g};base64,${Buffer.from(P).toString("base64")}`}catch{}};for(let E of d.nodes){A.lastIndex=0;let v;for(;(v=A.exec(E.content??""))!==null;)await C(v[1])}let T=J(d,f),w=l.Uri.joinPath(c,`${a}.html`);await l.workspace.fs.writeFile(w,Buffer.from(T,"utf-8"));let N=await l.window.showInformationMessage(`HTML exported: ${a}.html`,"Open in Browser","Show in Explorer");N==="Open in Browser"?l.env.openExternal(w):N==="Show in Explorer"&&l.commands.executeCommand("revealFileInOS",w)}catch(d){l.window.showErrorMessage(`HTML export failed: ${d}`)}else if(o.type==="saveImage")try{let{filename:d,webviewUri:c}=await q(r.webview,e.uri,o.data,o.ext??"png");r.webview.postMessage({type:"imageSaved",nodeId:o.nodeId,filename:d,webviewUri:c})}catch(d){l.window.showErrorMessage(`Failed to save image: ${d}`)}else if(o.type==="deleteImageFile")await V(e.uri,o.filename);else if(o.type==="reload")try{let d=await l.workspace.fs.readFile(e.uri),c=Buffer.from(d).toString("utf-8"),a=JSON.parse(c),m=j(r.webview,e.uri,a);r.webview.postMessage({type:"load",data:a,imageUris:m})}catch{s("load")}else if(o.type==="openHelp"){let d=l.Uri.joinPath(this.context.extensionUri,"README.md");l.commands.executeCommand("markdown.showPreviewToSide",d.with({fragment:"features"}))}}),u=l.workspace.onDidChangeTextDocument(o=>{o.document.uri.toString()===e.uri.toString()&&(this._pendingSaves.has(e.uri.toString())||s("externalChange"))});t._activeWebview=r.webview,r.onDidChangeViewState(o=>{o.webviewPanel.active&&(t._activeWebview=r.webview,r.webview.postMessage({type:"focusCanvas"}))}),r.onDidDispose(()=>{p.dispose(),u.dispose(),t._activeWebview===r.webview&&(t._activeWebview=null)})}_getHtmlForWebview(e){let r=e.asWebviewUri(l.Uri.joinPath(this.context.extensionUri,"dist","webview.js")),n=e.asWebviewUri(l.Uri.joinPath(this.context.extensionUri,"dist","katex","katex.min.css")),i=_();return`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
</html>`}};var I=O(require("vscode")),Q=O(require("child_process"));function R(t){try{return Q.execSync(t,{timeout:5e3,stdio:["pipe","pipe","pipe"]}).toString().trim()}catch{return""}}function y(t){return R(t)!==""}async function ee(t){if(!t||t.length===0)return;let e=[],r=new Date().toISOString(),n=process.platform,i=n==="win32"?"Windows":n==="darwin"?"macOS":"Linux",s=process.arch,p=R("python3 --version 2>&1")||R("python --version 2>&1"),u=y("python3 --version 2>&1")?"python3":y("python --version 2>&1")?"python":"",o=u!=="",d=o&&y(`${u} -c "import fitz" 2>&1 && echo ok`),c=d?R(`${u} -c "import fitz; print(fitz.__version__)"`):"",a=o&&y(`${u} -c "import pdfplumber" 2>&1 && echo ok`),m=o&&y(`${u} -c "import pdfminer" 2>&1 && echo ok`),f=o&&y(`${u} -c "from PIL import Image" 2>&1 && echo ok`),A=f?R(`${u} -c "from PIL import __version__; print(__version__)"`):"",C=o&&y(`${u} -c "import cv2" 2>&1 && echo ok`),T=y("pdftotext -v 2>&1 && echo ok")||y("pdftotext --help 2>&1 && echo ok"),w=y("convert --version 2>&1 && echo ok"),N=y("magick --version 2>&1 && echo ok"),E=y("gs --version 2>&1 && echo ok")||y("gswin64c --version 2>&1 && echo ok"),v=k=>k?"\u2705":"\u274C";e.push("# NodeGraph \u2014 Agent Environment Report"),e.push(""),e.push("> Auto-generated by the NodeGraph extension at activation."),e.push("> **AI agents: read this file to understand what tools are available on this machine.**"),e.push("> Re-generated each time a `.nodegraph.json` file is opened."),e.push(""),e.push(`Generated: \`${r}\``),e.push(""),e.push("---"),e.push(""),e.push("## System"),e.push(""),e.push("| | |"),e.push("|---|---|"),e.push(`| OS | ${i} (\`${n}\`) |`),e.push(`| Architecture | \`${s}\` |`),e.push(`| Python | ${o?`${v(!0)} \`${p}\``:`${v(!1)} not found`} |`),e.push(`| Python command | ${o?`\`${u}\``:"N/A"} |`),e.push(""),e.push("---"),e.push(""),e.push("## PDF Reading Capabilities"),e.push(""),e.push("| Tool | Available | Notes |"),e.push("|------|:---------:|-------|"),e.push(`| PyMuPDF (\`fitz\`) | ${v(d)} | ${d?`v${c} \u2014 recommended`:"Install: `pip install pymupdf`"} |`),e.push(`| pdfplumber | ${v(a)} | ${a?"available":"Install: `pip install pdfplumber`"} |`),e.push(`| pdfminer | ${v(m)} | ${m?"available":"Install: `pip install pdfminer.six`"} |`),e.push(`| poppler (\`pdftotext\`) | ${v(T)} | ${T?"CLI tool available":n==="win32"?"Install: download poppler for Windows":n==="darwin"?"Install: `brew install poppler`":"Install: `apt install poppler-utils`"} |`),e.push(`| Ghostscript (\`gs\`) | ${v(E)} | ${E?"available":"optional"} |`),e.push(""),e.push("---"),e.push(""),e.push("## Image Processing Capabilities"),e.push(""),e.push("| Tool | Available | Notes |"),e.push("|------|:---------:|-------|"),e.push(`| Pillow (\`PIL\`) | ${v(f)} | ${f?`v${A} \u2014 recommended`:"Install: `pip install Pillow`"} |`),e.push(`| OpenCV (\`cv2\`) | ${v(C)} | ${C?"available":"Install: `pip install opencv-python`"} |`),e.push(`| ImageMagick (\`convert\`) | ${v(w||N)} | ${w||N?"CLI tool available":n==="win32"?"Install: imagemagick.org":n==="darwin"?"Install: `brew install imagemagick`":"Install: `apt install imagemagick`"} |`),e.push(""),e.push("---"),e.push(""),e.push("## Agent Recommendations"),e.push(""),o||(e.push("> \u26A0\uFE0F **Python not found.** PDF reading and image processing via Python are not available."),e.push("> Install Python from https://python.org, then reopen a `.nodegraph.json` file to re-run this check."),e.push("")),e.push("### Reading a PDF"),d?(e.push("Use PyMuPDF (recommended \u2014 fastest and most accurate):"),e.push("```python"),e.push("import fitz"),e.push('doc = fitz.open("paper.pdf")'),e.push('text = "\\n".join(page.get_text() for page in doc)'),e.push("```")):a?(e.push("Use pdfplumber:"),e.push("```python"),e.push("import pdfplumber"),e.push('with pdfplumber.open("paper.pdf") as pdf:'),e.push('    text = "\\n".join(p.extract_text() or "" for p in pdf.pages)'),e.push("```")):T?(e.push("Use poppler CLI:"),e.push("```bash"),e.push("pdftotext paper.pdf -"),e.push("```")):e.push("\u274C No PDF reading tool available. Ask the user to install PyMuPDF: `pip install pymupdf`"),e.push(""),e.push("### Extracting images from a PDF"),d?(e.push("```python"),e.push("import fitz"),e.push('doc = fitz.open("paper.pdf")'),e.push("for i, page in enumerate(doc):"),e.push("    for img in page.get_images():"),e.push("        xref = img[0]"),e.push("        pix = fitz.Pixmap(doc, xref)"),e.push('        pix.save(f"fig_{i}_{xref}.png")'),e.push("```")):f?e.push("Pillow is available but cannot extract from PDF directly. Use PyMuPDF for extraction."):e.push("\u274C No image extraction tool available."),e.push(""),e.push("---"),e.push(""),e.push("*To refresh this report, reopen any `.nodegraph.json` file.*");let P=e.join(`
`);for(let k of t){let g=I.Uri.joinPath(k.uri,".agent"),S=I.Uri.joinPath(g,"ENVIRONMENT.md");try{await I.workspace.fs.createDirectory(g),await I.workspace.fs.writeFile(S,Buffer.from(P,"utf-8"))}catch{}}}async function te(t,e){let r=I.Uri.joinPath(t,".agent","NODEGRAPH_SPEC.md"),n;try{n=await I.workspace.fs.readFile(r)}catch{return!1}let i=I.Uri.joinPath(e,".agent"),s=I.Uri.joinPath(i,"NODEGRAPH_SPEC.md");try{return await I.workspace.fs.createDirectory(i),await I.workspace.fs.writeFile(s,n),!0}catch{return!1}}var xe=[{id:"tomoki1207.pdf",name:"vscode-pdf (PDF Viewer)"}];async function be(){for(let t of xe)if(!h.extensions.getExtension(t.id))try{await h.commands.executeCommand("workbench.extensions.installExtension",t.id)}catch{}}async function re(t){if(t)return t;let e=h.workspace.workspaceFolders??[];return e.length===0?void 0:e.length===1?e[0].uri:(await h.window.showWorkspaceFolderPick({placeHolder:"Select a folder for NodeGraph"}))?.uri}async function ye(t){let e=await re(t),r=e?h.Uri.joinPath(e,"untitled.nodegraph.json"):void 0,n=await h.window.showSaveDialog({defaultUri:r,filters:{NodeGraph:["nodegraph.json"]},title:"Create New NodeGraph"});if(!n)return;let i=n.fsPath.endsWith(".nodegraph.json")?n:n.with({path:n.path.replace(/(\.nodegraph)?(\.json)?$/,"")+".nodegraph.json"}),s=F();await h.workspace.fs.writeFile(i,Buffer.from(JSON.stringify(s,null,2),"utf-8")),await h.commands.executeCommand("vscode.openWith",i,"nodegraph.editor")}function we(t){t.subscriptions.push(M.register(t)),t.subscriptions.push(h.commands.registerCommand("nodegraph.search",()=>{M.postToActive({type:"openSearch"})}),h.commands.registerCommand("nodegraph.fitView",()=>{M.postToActive({type:"fitView"})}),h.commands.registerCommand("nodegraph.collapseAll",()=>{M.postToActive({type:"collapseAll"})}),h.commands.registerCommand("nodegraph.expandAll",()=>{M.postToActive({type:"expandAll"})}),h.commands.registerCommand("nodegraph.new",e=>ye(e))),ee(h.workspace.workspaceFolders??[]),t.subscriptions.push(h.commands.registerCommand("nodegraph.copyAgentSpec",async e=>{let r=await re(e);if(!r){h.window.showWarningMessage("NodeGraph: open or select a folder first \u2014 there is no workspace to copy the spec into.");return}await te(t.extensionUri,r)?h.window.showInformationMessage(`NodeGraph: copied NODEGRAPH_SPEC.md to .agent/ in ${r.fsPath}.`):h.window.showErrorMessage("NodeGraph: failed to copy the agent spec \u2014 check that the folder is writable and try again.")})),be()}function Ee(){}0&&(module.exports={activate,deactivate});
