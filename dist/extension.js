...
async function createNewGraph() {
  const wsFolder = vscode4.workspace.workspaceFolders?.[0]?.uri;
  const defaultUri = wsFolder ? vscode4.Uri.joinPath(wsFolder, "untitled.nodegraph.json") : void 0;
  const picked = await vscode4.window.showSaveDialog({
    defaultUri,
  ...
  if (!picked)
    return;
  const uri = picked.fsPath.endsWith(".nodegraph.json") ? picked : picked.with({ path: picked.path.replace(/(\.nodegraph)?(\.json)?$/, "") + ".nodegraph.json" });
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const starter = {
    version: "1.0.0",
    title: "New Graph",
  ...
  await vscode4.workspace.fs.writeFile(uri, Buffer.from(JSON.stringify(starter, null, 2), "utf-8"));
  await vscode4.commands.executeCommand("vscode.openWith", uri, "nodegraph.editor");
}
function activate(context) {
  ...
  context.subscriptions.push(
    ...
    vscode4.commands.registerCommand("nodegraph.fitView", () => {
      NodeGraphEditorProvider.postToActive({ type: "fitView" });
    }),
    vscode4.commands.registerCommand("nodegraph.collapseAll", () => {
      NodeGraphEditorProvider.postToActive({ type: "collapseAll" });
    }),
    vscode4.commands.registerCommand("nodegraph.expandAll", () => {
      NodeGraphEditorProvider.postToActive({ type: "expandAll" });
    }),
    vscode4.commands.registerCommand("nodegraph.new", () => createNewGraph())
  );
