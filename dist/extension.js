"use strict";var be=Object.create;var B=Object.defineProperty;var ye=Object.getOwnPropertyDescriptor;var we=Object.getOwnPropertyNames;var Ee=Object.getPrototypeOf,Ie=Object.prototype.hasOwnProperty;var ke=(e,t)=>{for(var r in t)B(e,r,{get:t[r],enumerable:!0})},ee=(e,t,r,n)=>{if(t&&typeof t=="object"||typeof t=="function")for(let o of we(t))!Ie.call(e,o)&&o!==r&&B(e,o,{get:()=>t[o],enumerable:!(n=ye(t,o))||n.enumerable});return e};var C=(e,t,r)=>(r=e!=null?be(Ee(e)):{},ee(t||!e||!e.__esModule?B(r,"default",{value:e,enumerable:!0}):r,e)),Se=e=>ee(B({},"__esModule",{value:!0}),e);var Ye={};ke(Ye,{activate:()=>He,deactivate:()=>_e});module.exports=Se(Ye);var m=C(require("vscode"));var a=C(require("vscode")),pe=C(require("path"));var T=C(require("vscode"));function U(e){let t=T.Uri.joinPath(e,".."),r=e.path.split("/").pop()?.replace(/\.nodegraph\.json$/,"")??"graph";return T.Uri.joinPath(t,`.${r}-imgs`)}function Ae(e,t,r){let n=T.Uri.joinPath(U(t),r);return e.asWebviewUri(n).toString()}var te=/\[\[IMG:([^:\]]+)(?::[^\]]+)?\]\]/g;function j(e,t,r){let n={},o=s=>{s&&!n[s]&&(n[s]=Ae(e,t,s))};for(let s of r.nodes){te.lastIndex=0;let c;for(;(c=te.exec(s.content??""))!==null;)o(c[1])}for(let s of r.canvasImages??[])o(s.filename);return n}async function re(e,t,r,n="png"){let o=U(t);try{await T.workspace.fs.createDirectory(o)}catch{}let s=`img_${Date.now()}.${n}`,c=T.Uri.joinPath(o,s);return await T.workspace.fs.writeFile(c,Buffer.from(r,"base64")),{filename:s,webviewUri:e.asWebviewUri(c).toString()}}async function ne(e,t){let r=T.Uri.joinPath(U(e),t);try{await T.workspace.fs.delete(r)}catch{}}var ie=C(require("path"));function $(e){let t=e.match(/^(.+):(\d+)(?:-(\d+))?$/);return t?{path:t[1],startLine:parseInt(t[2],10),endLine:t[3]?parseInt(t[3],10):void 0}:{path:e}}function y(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function De(e){let t=e.trim().replace("#",""),r=t.length===3?t.split("").map(n=>n+n).join(""):t;return/^[0-9a-fA-F]{6}$/.test(r)?{r:255-parseInt(r.slice(0,2),16),g:255-parseInt(r.slice(2,4),16),b:255-parseInt(r.slice(4,6),16)}:null}var Me=e=>e.replace(/[^a-zA-Z0-9_-]/g,"_");function L(e){return/^\s*\|/.test(e)&&e.indexOf("|",1)!==-1}function q(e){return/^\s*\|[\s\-:|]+\|\s*$/.test(e)&&!/[a-zA-Z0-9]/.test(e)}function oe(e){return e.replace(/^\s*\|/,"").replace(/\|\s*$/,"").split("|").map(t=>t.trim())}function Ce(e){if(!e)return[{type:"text",text:"",startChar:0,endChar:0}];let t=e.split(`
`),r=[],n=0,o=0,s=c=>t[c].length+(c<t.length-1?1:0);for(;n<t.length;)if(L(t[n])&&n+1<t.length&&q(t[n+1])){let f=o,i=[];for(;n<t.length&&L(t[n]);)i.push(t[n]),o+=s(n),n++;i.length>=3?r.push({type:"table",headers:oe(i[0]),rows:i.slice(2).map(oe),startChar:f,endChar:o}):r.push({type:"text",text:i.join(`
`),startChar:f,endChar:o})}else{let f=o,i=[];for(;n<t.length&&!(L(t[n])&&n+1<t.length&&q(t[n+1]));)i.push(t[n]),o+=s(n),n++;r.push({type:"text",text:i.join(`
`),startChar:f,endChar:o})}return r}function G(e){let t=e.split(`
`);for(let r=0;r+1<t.length;r++)if(L(t[r])&&q(t[r+1]))return!0;return!1}function Te(e){return y(e).replace(/\\\$/g,()=>'<span class="ng-cur">$</span>')}function F(e){return Te(e).replace(/\*\*(.+?)\*\*/g,'<strong style="font-size:1.1em">$1</strong>')}function H(e,t){let r=/\[\[IMG:([^:\]]+)(?::(\d+)x(\d+))?\]\]/g,n="",o=0,s;for(;(s=r.exec(e))!==null;){s.index>o&&(n+=F(e.slice(o,s.index)));let c=s[1],f=s[2],i=s[3],l=f&&i?` width="${f}" height="${i}"`:"",u=t[c];n+=u?`<img class="ng-img${l?" ng-img-sized":""}" src="${u}"${l} alt="${y(c)}" onclick="showLightbox(this.src)" title="Click to enlarge">`:`<span class="ng-img-missing">${y(c)}</span>`,o=s.index+s[0].length}return o<e.length&&(n+=F(e.slice(o))),n}function Ne(e,t){let r=e.headers.map(o=>`<th>${H(o,t)}</th>`).join(""),n=e.rows.map(o=>`<tr>${o.map(s=>`<td>${H(s,t)}</td>`).join("")}</tr>`).join("");return`<div class="ng-table-wrap"><table class="ng-table"><thead><tr>${r}</tr></thead><tbody>${n}</tbody></table></div>`}function Pe(e,t,r,n,o,s){let c=t?.color??"#888",f=t?.shape==="rounded"?"22px":"2px",i=y(t?.label??e.template),l=Math.round(e.position.x+r),u=Math.round(e.position.y+n),p="",d=e.content??"";if(G(d)){let h=Ce(d);p+='<div class="ng-content">';for(let D of h)D.type==="table"?p+=Ne(D,o):D.text&&(p+=`<div class="ng-seg">${H(D.text,o).replace(/\n/g,"<br>")}</div>`);p+="</div>"}else d&&(p+=`<div class="ng-content">${H(d,o).replace(/\n/g,"<br>")}</div>`);if(e.original){let h=y(e.original.title??"Original"),D=e.originalExpanded?" open":"";p+=`<details class="ng-original"${D}><summary>${h}${e.original.location?` <span class="ng-loc">${y(e.original.location)}</span>`:""}</summary>
<div class="ng-orig-text">${F(e.original.text).replace(/\n/g,"<br>")}</div></details>`}for(let h of e.toggleItems??[])p+=`<details class="ng-toggle" data-toggle-id="${y(h.id)}"${h.expanded?" open":""}><summary>${y(h.title||"(untitled)")}</summary>
<div class="ng-toggle-body">${F(h.content).replace(/\n/g,"<br>")}</div></details>`;e.links.length&&(p+=`<div class="ng-links">${e.links.map(h=>{let D=h.type==="url"?"\u{1F517}":h.type==="pdf"?"\u{1F4C4}":h.type==="obsidian"?"\u{1F7E3}":h.type==="code"?"\u{1F4BB}":"\u2B21";if(h.type==="code"){let{path:K,startLine:J,endLine:Z}=$(h.target),Q=ie.posix.normalize(s.repoPrefix+K),ve=/\.ipynb$/i.test(K),xe=J&&!ve?`#L${J}${Z?`-L${Z}`:""}`:"";return`<a class="ng-link"${s.githubBase&&!Q.startsWith("..")?` href="${y(s.githubBase)}/${y(Q)}${xe}" target="_blank"`:""}>${D} ${y(h.label||h.target)}</a>`}return`<a class="ng-link"${h.type==="url"||h.type==="pdf"?` href="${y(h.target)}" target="_blank"`:""}>${D} ${y(h.label||h.target)}</a>`}).join("")}</div>`);let b=!!p,g=e.contentExpanded?"":' style="display:none"',w=e.children.length?` data-children="${e.children.join(",")}"`:"",E=G(d)?" ng-has-table":"",A=/\[\[IMG:[^:\]]+:(\d+)x\d+\]\]/g,I=0,v;for(;(v=A.exec(d))!==null;)I=Math.max(I,Number(v[1]));let O=I>0?G(d)?I+280:I+32:0,M=Math.max(e.nodeWidth??0,432,O),P=[M>432?`min-width:${M}px`:"",e.nodeHeight&&e.contentExpanded?`min-height:${e.nodeHeight}px`:""].filter(Boolean).join(";"),W=e.nodeHeight?` data-min-h="${e.nodeHeight}"`:"";return`<div class="ng-node${E}" id="node-${y(e.id)}"${w}${W} style="--color:${c};border-radius:${f};left:${l}px;top:${u}px${P?";"+P:""}">
  ...
  ${b?`<div class="ng-body"${g}${e.fontSize?` style="font-size:${e.fontSize}px"`:""}>${p}</div>`:""}
</div>`}function ae(e,t={},r={githubBase:null,repoPrefix:""}){let n=1/0,o=1/0;for(let d of e.nodes)n=Math.min(n,d.position.x),o=Math.min(o,d.position.y);isFinite(n)||(n=0,o=0);let s=-n+100,c=-o+100,f=e.nodes.map(d=>Pe(d,e.nodeTemplates[d.template],s,c,t,r)).join(`
`),i=JSON.stringify(e.nodes.map(d=>({id:d.id,lx:Math.round(d.position.x+s),ly:Math.round(d.position.y+c),children:d.children??[],template:d.template,contentExpanded:d.contentExpanded,isMain:d.template==="main_topic",nodeHeight:d.nodeHeight??null,naturalY:Math.round((d.nodeNaturalY??d.position.y)+c),title:d.title,content:d.content??"",originalTitle:d.original?.title??"",originalText:d.original?.text??"",toggles:(d.toggleItems??[]).map(b=>({id:b.id,title:b.title,content:b.content}))}))),l=JSON.stringify(e.edges.map(d=>({source:d.source,target:d.target,type:d.type,label:d.label||""}))),u=JSON.stringify(Object.fromEntries(Object.entries(e.nodeTemplates).map(([d,b])=>[d,b.label]))),p=Object.entries(e.nodeTemplates).map(([d,b])=>{let g=De(b.color),w=g?`rgb(${g.r},${g.g},${g.b})`:"#ff3b30",E=g?`rgba(${g.r},${g.g},${g.b},0.18)`:"rgba(255,59,48,0.18)";return`::highlight(ng-hit-${Me(d)}){color:${w};background-color:${E};text-decoration:underline}`}).join(`
`);return`<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
window.addEventListener('load', function() {
  ...
  imgs.forEach(function(img) {
...
</html>`}var Oe={main_topic:{label:"Main topic",color:"#4B8BBE",icon:"file-text",shape:"sharp"},method:{label:"Method",color:"#5C9E6E",icon:"cpu",shape:"sharp"},result:{label:"Result",color:"#9B59B6",icon:"bar-chart-2",shape:"sharp"},claim:{label:"Claim",color:"#E74C3C",icon:"alert-circle",shape:"sharp"},question:{label:"Question",color:"#E5A835",icon:"help-circle",shape:"rounded"},gap:{label:"Gap / Idea",color:"#1ABC9C",icon:"lightbulb",shape:"rounded"},reference:{label:"Reference",color:"#95A5A6",icon:"book-open",shape:"rounded"},memo:{label:"Memo",color:"#BDC3C7",icon:"edit-3",shape:"rounded"}};function _(e="New Graph"){let t=new Date().toISOString();return{version:"1.0.0",title:e,created:t,modified:t,nodeTemplates:Oe,nodes:[],edges:[],viewport:{x:0,y:0,zoom:1}}}function Y(){let e="",t="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";for(let r=0;r<32;r++)e+=t.charAt(Math.floor(Math.random()*t.length));return e}var k=C(require("vscode")),se=C(require("fs"));var X=class e{static{this.panels=new Map}static async openAndSearch(t,r,n,o){let s=r.toString(),c=e.panels.get(s);if(c){c.panel.reveal(k.ViewColumn.Beside,!0),c.ready?c.panel.webview.postMessage({type:"search",query:n,pageHint:o}):c.pending={query:n,pageHint:o};return}let f;try{f=await k.workspace.fs.readFile(r)}catch{k.window.showErrorMessage(`PDF\uB97C \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4: ${r.fsPath}`);return}let i=k.window.createWebviewPanel("nodegraph.pdfViewer",r.path.split("/").pop()??"PDF",{viewColumn:k.ViewColumn.Beside,preserveFocus:!1},{enableScripts:!0,retainContextWhenHidden:!0,localResourceRoots:[k.Uri.joinPath(t.extensionUri,"dist")]}),l={panel:i,ready:!1,pending:{query:n,pageHint:o}};e.panels.set(s,l),i.iconPath=k.Uri.joinPath(t.extensionUri,"resources","icon-hires.png"),i.webview.html=e._getHtml(t,i.webview);let u=Buffer.from(f).toString("base64");i.webview.onDidReceiveMessage(p=>{p.type==="ready"&&(l.ready=!0,i.webview.postMessage({type:"load",pdfData:u,query:l.pending?.query,pageHint:l.pending?.pageHint}),l.pending=null)}),i.onDidDispose(()=>{e.panels.delete(s)})}static _getHtml(t,r){let n=k.Uri.joinPath(t.extensionUri,"dist","pdfjs-viewer","web"),o=r.asWebviewUri(n).toString()+"/",s=r.asWebviewUri(k.Uri.joinPath(t.extensionUri,"dist","pdfjs-viewer","build","pdf.worker.min.mjs")),c=k.Uri.joinPath(n,"viewer.html").fsPath,f=Y(),i=`default-src 'none'; img-src ${r.cspSource} data: blob:; script-src 'nonce-${f}' ${r.cspSource}; style-src 'unsafe-inline' ${r.cspSource}; worker-src ${r.cspSource} blob:; connect-src ${r.cspSource} blob:;`;return se.readFileSync(c,"utf-8").replace('<meta charset="utf-8">',`<meta charset="utf-8">
    ...
    </script>`)}};var de=C(require("child_process"));function z(e,t){try{return de.execSync(e,{cwd:t,timeout:5e3,stdio:["pipe","pipe","pipe"]}).toString().trim()}catch{return""}}function Re(e){let t=e.match(/github\.com[:/]([^/]+)\/(.+?)(?:\.git)?$/);return t?`${t[1]}/${t[2]}`:null}function le(e){let t=z("git remote get-url origin",e);if(!t)return null;let r=Re(t);if(!r)return null;let n=z("git rev-parse HEAD",e);return n?`https://github.com/${r}/blob/${n}`:null}function ce(e){let t=z("git rev-parse --show-toplevel",e);if(!t)return"";let r=e.replace(/\\/g,"/").replace(t.replace(/\\/g,"/"),"").replace(/^\/+/,"");return r?`${r}/`:""}var N=class e{constructor(t){this.context=t;this._pendingSaves=new Set}static register(t){let r=new e(t);return a.window.registerCustomEditorProvider("nodegraph.editor",r,{webviewOptions:{retainContextWhenHidden:!0}})}static{this._activeWebview=null}static postToActive(t){e._activeWebview?.postMessage(t)}async resolveCustomTextEditor(t,r,n){r.iconPath=a.Uri.joinPath(this.context.extensionUri,"resources","icon-hires.png");let o=a.Uri.joinPath(t.uri,"..");r.webview.options={enableScripts:!0,localResourceRoots:[this.context.extensionUri,o]},r.webview.html=this._getHtmlForWebview(r.webview);let s=i=>{let l=t.getText();try{let u=l.trim()===""?_():JSON.parse(l),p=j(r.webview,t.uri,u);r.webview.postMessage({type:i,data:u,imageUris:p})}catch{}},c=r.webview.onDidReceiveMessage(async i=>{if(i.type==="ready")s("load");else if(i.type==="save"){let l=t.uri.toString();this._pendingSaves.add(l);try{let u=new a.WorkspaceEdit,p=new a.Range(t.positionAt(0),t.positionAt(t.getText().length));u.replace(t.uri,p,JSON.stringify(i.data,null,2)),await a.workspace.applyEdit(u),await t.save()}finally{this._pendingSaves.delete(l)}}else if(i.type==="openLink"){let l=i.link;if(l.type==="url")a.env.openExternal(a.Uri.parse(l.target));else if(l.type==="pdf"){let u=a.Uri.joinPath(a.Uri.joinPath(t.uri,".."),l.target);a.env.openExternal(u)}else if(l.type==="obsidian")a.env.openExternal(a.Uri.parse(l.target));else if(l.type==="code"){let{path:u,startLine:p,endLine:d}=$(l.target);try{let b=a.Uri.joinPath(a.Uri.joinPath(t.uri,".."),u);if(u.toLowerCase().endsWith(".ipynb")){let g=await a.workspace.openNotebookDocument(b),w=await a.window.showNotebookDocument(g,{preview:!1});if(p){let E=Math.min(Math.max(0,p-1),g.cellCount-1),A=Math.min(Math.max(E,(d??p)-1),g.cellCount-1),I=new a.NotebookRange(E,A+1);w.selection=I,w.revealRange(I,a.NotebookEditorRevealType.InCenter)}}else{let g=await a.workspace.openTextDocument(b),w=await a.window.showTextDocument(g,{preview:!1});if(p){let E=Math.max(0,p-1),A=Math.max(E,(d??p)-1),I=g.lineAt(Math.min(A,g.lineCount-1)).text.length,v=new a.Range(E,0,A,I);w.selection=new a.Selection(v.start,v.end),w.revealRange(v,a.TextEditorRevealType.InCenter)}}}catch{a.window.showErrorMessage(`NodeGraph: couldn't open ${u}`)}}}else if(i.type==="searchInPdf"){let l=a.Uri.joinPath(a.Uri.joinPath(t.uri,".."),i.pdfTarget);X.openAndSearch(this.context,l,i.query,i.pageHint)}else if(i.type==="exportHtml")try{let l=i.data,u=a.Uri.joinPath(t.uri,".."),p=pe.basename(t.uri.fsPath,".nodegraph.json"),d=a.Uri.joinPath(u,`.${p}-imgs`),b={},g=/\[\[IMG:([^:\]]+)(?::[^\]]+)?\]\]/g,w=async M=>{if(!(!M||b[M]))try{let P=a.Uri.joinPath(d,M),W=await a.workspace.fs.readFile(P),h=M.split(".").pop()?.toLowerCase()??"png",D=h==="jpg"||h==="jpeg"?"image/jpeg":h==="gif"?"image/gif":h==="webp"?"image/webp":"image/png";b[M]=`data:${D};base64,${Buffer.from(W).toString("base64")}`}catch{}};for(let M of l.nodes){g.lastIndex=0;let P;for(;(P=g.exec(M.content??""))!==null;)await w(P[1])}let E=le(u.fsPath),A=E?ce(u.fsPath):"",I=ae(l,b,{githubBase:E,repoPrefix:A}),v=a.Uri.joinPath(u,`${p}.html`);await a.workspace.fs.writeFile(v,Buffer.from(I,"utf-8"));let O=await a.window.showInformationMessage(`HTML exported: ${p}.html`,"Open in Browser","Show in Explorer");O==="Open in Browser"?a.env.openExternal(v):O==="Show in Explorer"&&a.commands.executeCommand("revealFileInOS",v)}catch(l){a.window.showErrorMessage(`HTML export failed: ${l}`)}else if(i.type==="saveImage")try{let{filename:l,webviewUri:u}=await re(r.webview,t.uri,i.data,i.ext??"png");r.webview.postMessage({type:"imageSaved",nodeId:i.nodeId,filename:l,webviewUri:u})}catch(l){a.window.showErrorMessage(`Failed to save image: ${l}`)}else if(i.type==="deleteImageFile")await ne(t.uri,i.filename);else if(i.type==="reload")try{let l=await a.workspace.fs.readFile(t.uri),u=Buffer.from(l).toString("utf-8"),p=JSON.parse(u),d=j(r.webview,t.uri,p);r.webview.postMessage({type:"load",data:p,imageUris:d})}catch{s("load")}else if(i.type==="openHelp"){let l=a.Uri.joinPath(this.context.extensionUri,"README.md");a.commands.executeCommand("markdown.showPreviewToSide",l.with({fragment:"features"}))}}),f=a.workspace.onDidChangeTextDocument(i=>{i.document.uri.toString()===t.uri.toString()&&(this._pendingSaves.has(t.uri.toString())||s("externalChange"))});e._activeWebview=r.webview,r.onDidChangeViewState(i=>{i.webviewPanel.active&&(e._activeWebview=r.webview,r.webview.postMessage({type:"focusCanvas"}))}),r.onDidDispose(()=>{c.dispose(),f.dispose(),e._activeWebview===r.webview&&(e._activeWebview=null)})}_getHtmlForWebview(t){let r=t.asWebviewUri(a.Uri.joinPath(this.context.extensionUri,"dist","webview.js")),n=t.asWebviewUri(a.Uri.joinPath(this.context.extensionUri,"dist","katex","katex.min.css")),o=Y();return`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
</html>`}};var x=C(require("vscode")),ue=C(require("child_process"));function R(e){try{return ue.execSync(e,{timeout:5e3,stdio:["pipe","pipe","pipe"]}).toString().trim()}catch{return""}}function S(e){return R(e)!==""}function Be(){let e=[],t=new Date().toISOString(),r=process.platform,n=r==="win32"?"Windows":r==="darwin"?"macOS":"Linux",o=process.arch,s=R("python3 --version 2>&1")||R("python --version 2>&1"),c=S("python3 --version 2>&1")?"python3":S("python --version 2>&1")?"python":"",f=c!=="",i=f&&S(`${c} -c "import fitz" 2>&1 && echo ok`),l=i?R(`${c} -c "import fitz; print(fitz.__version__)"`):"",u=f&&S(`${c} -c "import pdfplumber" 2>&1 && echo ok`),p=f&&S(`${c} -c "import pdfminer" 2>&1 && echo ok`),d=f&&S(`${c} -c "from PIL import Image" 2>&1 && echo ok`),b=d?R(`${c} -c "from PIL import __version__; print(__version__)"`):"",g=f&&S(`${c} -c "import cv2" 2>&1 && echo ok`),w=S("pdftotext -v 2>&1 && echo ok")||S("pdftotext --help 2>&1 && echo ok"),E=S("convert --version 2>&1 && echo ok"),A=S("magick --version 2>&1 && echo ok"),I=S("gs --version 2>&1 && echo ok")||S("gswin64c --version 2>&1 && echo ok"),v=O=>O?"\u2705":"\u274C";return e.push("# NodeGraph \u2014 Agent Environment Report"),e.push(""),e.push("> Auto-generated by the NodeGraph extension at activation."),e.push("> **AI agents: read this file to understand what tools are available on this machine.**"),e.push("> Re-generated each time a `.nodegraph.json` file is opened."),e.push(""),e.push(`Generated: \`${t}\``),e.push(""),e.push("---"),e.push(""),e.push("## System"),e.push(""),e.push("| | |"),e.push("|---|---|"),e.push(`| OS | ${n} (\`${r}\`) |`),e.push(`| Architecture | \`${o}\` |`),e.push(`| Python | ${f?`${v(!0)} \`${s}\``:`${v(!1)} not found`} |`),e.push(`| Python command | ${f?`\`${c}\``:"N/A"} |`),e.push(""),e.push("---"),e.push(""),e.push("## PDF Reading Capabilities"),e.push(""),e.push("| Tool | Available | Notes |"),e.push("|------|:---------:|-------|"),e.push(`| PyMuPDF (\`fitz\`) | ${v(i)} | ${i?`v${l} \u2014 recommended`:"Install: `pip install pymupdf`"} |`),e.push(`| pdfplumber | ${v(u)} | ${u?"available":"Install: `pip install pdfplumber`"} |`),e.push(`| pdfminer | ${v(p)} | ${p?"available":"Install: `pip install pdfminer.six`"} |`),e.push(`| poppler (\`pdftotext\`) | ${v(w)} | ${w?"CLI tool available":r==="win32"?"Install: download poppler for Windows":r==="darwin"?"Install: `brew install poppler`":"Install: `apt install poppler-utils`"} |`),e.push(`| Ghostscript (\`gs\`) | ${v(I)} | ${I?"available":"optional"} |`),e.push(""),e.push("---"),e.push(""),e.push("## Image Processing Capabilities"),e.push(""),e.push("| Tool | Available | Notes |"),e.push("|------|:---------:|-------|"),e.push(`| Pillow (\`PIL\`) | ${v(d)} | ${d?`v${b} \u2014 recommended`:"Install: `pip install Pillow`"} |`),e.push(`| OpenCV (\`cv2\`) | ${v(g)} | ${g?"available":"Install: `pip install opencv-python`"} |`),e.push(`| ImageMagick (\`convert\`) | ${v(E||A)} | ${E||A?"CLI tool available":r==="win32"?"Install: imagemagick.org":r==="darwin"?"Install: `brew install imagemagick`":"Install: `apt install imagemagick`"} |`),e.push(""),e.push("---"),e.push(""),e.push("## Agent Recommendations"),e.push(""),f||(e.push("> \u26A0\uFE0F **Python not found.** PDF reading and image processing via Python are not available."),e.push("> Install Python from https://python.org, then reopen a `.nodegraph.json` file to re-run this check."),e.push("")),e.push("### Reading a PDF"),i?(e.push("Use PyMuPDF (recommended \u2014 fastest and most accurate):"),e.push("```python"),e.push("import fitz"),e.push('doc = fitz.open("paper.pdf")'),e.push('text = "\\n".join(page.get_text() for page in doc)'),e.push("```")):u?(e.push("Use pdfplumber:"),e.push("```python"),e.push("import pdfplumber"),e.push('with pdfplumber.open("paper.pdf") as pdf:'),e.push('    text = "\\n".join(p.extract_text() or "" for p in pdf.pages)'),e.push("```")):w?(e.push("Use poppler CLI:"),e.push("```bash"),e.push("pdftotext paper.pdf -"),e.push("```")):e.push("\u274C No PDF reading tool available. Ask the user to install PyMuPDF: `pip install pymupdf`"),e.push(""),e.push("### Extracting images from a PDF"),i?(e.push("```python"),e.push("import fitz"),e.push('doc = fitz.open("paper.pdf")'),e.push("for i, page in enumerate(doc):"),e.push("    for img in page.get_images():"),e.push("        xref = img[0]"),e.push("        pix = fitz.Pixmap(doc, xref)"),e.push('        pix.save(f"fig_{i}_{xref}.png")'),e.push("```")):d?e.push("Pillow is available but cannot extract from PDF directly. Use PyMuPDF for extraction."):e.push("\u274C No image extraction tool available."),e.push(""),e.push("---"),e.push(""),e.push("*To refresh this report, reopen any `.nodegraph.json` file.*"),e.join(`
`)}async function V(e){let t=x.Uri.joinPath(e,".agent"),r=x.Uri.joinPath(t,"ENVIRONMENT.md");try{return await x.workspace.fs.createDirectory(t),await x.workspace.fs.writeFile(r,Buffer.from(Be(),"utf-8")),!0}catch{return!1}}async function fe(e){if(!(!e||e.length===0))for(let t of e)await V(t.uri)}async function he(e,t){let r=x.Uri.joinPath(e,".agent","NODEGRAPH_SPEC.md"),n;try{n=await x.workspace.fs.readFile(r)}catch{return!1}let o=x.Uri.joinPath(t,".agent"),s=x.Uri.joinPath(o,"NODEGRAPH_SPEC.md");try{return await x.workspace.fs.createDirectory(o),await x.workspace.fs.writeFile(s,n),!0}catch{return!1}}async function ge(e,t){let r=x.Uri.joinPath(e,".prompt"),n=x.Uri.joinPath(t,".prompt");try{for(let o of["paper","lecture","code"]){let s=x.Uri.joinPath(n,o);await x.workspace.fs.createDirectory(s);for(let c of["korean.md","english.md"]){let f=await x.workspace.fs.readFile(x.Uri.joinPath(r,o,c));await x.workspace.fs.writeFile(x.Uri.joinPath(s,c),f)}}return!0}catch{return!1}}var $e=[{id:"tomoki1207.pdf",name:"vscode-pdf (PDF Viewer)"}];async function Le(){for(let e of $e)if(!m.extensions.getExtension(e.id))try{await m.commands.executeCommand("workbench.extensions.installExtension",e.id)}catch{}}async function me(e){if(e)return e;let t=m.workspace.workspaceFolders??[];return t.length===0?void 0:t.length===1?t[0].uri:(await m.window.showWorkspaceFolderPick({placeHolder:"Select a folder for NodeGraph"}))?.uri}async function Fe(e){let t=await me(e),r=t?m.Uri.joinPath(t,"untitled.nodegraph.json"):void 0,n=await m.window.showSaveDialog({defaultUri:r,filters:{NodeGraph:["nodegraph.json"]},title:"Create New NodeGraph"});if(!n)return;let o=n.fsPath.endsWith(".nodegraph.json")?n:n.with({path:n.path.replace(/(\.nodegraph)?(\.json)?$/,"")+".nodegraph.json"}),s=_();await m.workspace.fs.writeFile(o,Buffer.from(JSON.stringify(s,null,2),"utf-8")),await m.commands.executeCommand("vscode.openWith",o,"nodegraph.editor")}function He(e){e.subscriptions.push(N.register(e)),e.subscriptions.push(m.commands.registerCommand("nodegraph.search",()=>{N.postToActive({type:"openSearch"})}),m.commands.registerCommand("nodegraph.fitView",()=>{N.postToActive({type:"fitView"})}),m.commands.registerCommand("nodegraph.collapseAll",()=>{N.postToActive({type:"collapseAll"})}),m.commands.registerCommand("nodegraph.expandAll",()=>{N.postToActive({type:"expandAll"})}),m.commands.registerCommand("nodegraph.new",t=>Fe(t))),fe(m.workspace.workspaceFolders??[]),e.subscriptions.push(m.commands.registerCommand("nodegraph.copyAgentSpec",async t=>{let r=await me(t);if(!r){m.window.showWarningMessage("NodeGraph: open or select a folder first \u2014 there is no workspace to copy the spec into.");return}let n=await he(e.extensionUri,r),o=await V(r),s=await ge(e.extensionUri,r);n&&o&&s?m.window.showInformationMessage(`NodeGraph: wrote .agent/NODEGRAPH_SPEC.md, .agent/ENVIRONMENT.md, and .prompt/{paper,lecture,code}/{korean,english}.md in ${r.fsPath}.`):m.window.showErrorMessage("NodeGraph: failed to write the agent files \u2014 check that the folder is writable and try again.")})),Le()}function _e(){}0&&(module.exports={activate,deactivate});
