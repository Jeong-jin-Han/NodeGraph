...
<div class="ng-toggle-body">${L($(g.content))}</div></details>`;e.links.length&&(u+=`<div class="ng-links">${e.links.map(g=>{let C=g.type==="url"?"\u{1F517}":g.type==="pdf"?"\u{1F4C4}":g.type==="obsidian"?"\u{1F7E3}":g.type==="code"?"\u{1F4BB}":"\u2B21";if(g.type==="code"){let{path:K,startLine:Z,endLine:Q}=F(g.target),ee=de.posix.normalize(l.repoPrefix+K),ye=/\.ipynb$/i.test(K),we=Z&&!ye?`#L${Z}${Q?`-L${Q}`:""}`:"";return`<a class="ng-link"${l.githubBase&&!ee.startsWith("..")?` href="${E(l.githubBase)}/${E(ee)}${we}" target="_blank"`:""}>${C} ${E(g.label||g.target)}</a>`}return`<a class="ng-link"${g.type==="url"||g.type==="pdf"?` href="${E(g.target)}" target="_blank"`:""}>${C} ${E(g.label||g.target)}</a>`}).join("")}</div>`);let b=!!u,h=e.contentExpanded?"":' style="display:none"',x=e.children.length?` data-children="${e.children.join(",")}"`:"",y=z(d)?" ng-has-table":"",A=/\[\[IMG:[^:\]]+:(\d+)x\d+\]\]/g,I=0,S;for(;(S=A.exec(d))!==null;)I=Math.max(I,Number(S[1]));let w=I>0?z(d)?I+280:I+32:0,T=Math.max(e.nodeWidth??0,432,w),P=[T>432?`min-width:${T}px`:"",e.nodeHeight&&e.contentExpanded?`min-height:${e.nodeHeight}px`:""].filter(Boolean).join(";"),U=e.nodeHeight?` data-min-h="${e.nodeHeight}"`:"";return`<div class="ng-node${y}" id="node-${E(e.id)}"${x}${U} style="--color:${c};border-radius:${f};left:${a}px;top:${p}px${P?";"+P:""}">
  <div class="ng-header" onclick="onHeaderClick(this)" title="Click to select node">
    ...
    ${b?`<span class="ng-title" onclick="onTitleClick(event,this)" title="Click to fold/unfold">${E(e.title)}</span>`:`<span class="ng-title">${E(e.title)}</span>`}
  </div>
  ${b?`<div class="ng-body"${h}${e.fontSize?` style="font-size:${e.fontSize}px"`:""}>${u}</div>`:""}
