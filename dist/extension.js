"use strict";var me=Object.create;var B=Object.defineProperty;var ve=Object.getOwnPropertyDescriptor;var xe=Object.getOwnPropertyNames;var be=Object.getPrototypeOf,ye=Object.prototype.hasOwnProperty;var we=(e,t)=>{for(var r in t)B(e,r,{get:t[r],enumerable:!0})},Q=(e,t,r,n)=>{if(t&&typeof t=="object"||typeof t=="function")for(let o of xe(t))!ye.call(e,o)&&o!==r&&B(e,o,{get:()=>t[o],enumerable:!(n=ve(t,o))||n.enumerable});return e};var C=(e,t,r)=>(r=e!=null?me(be(e)):{},Q(t||!e||!e.__esModule?B(r,"default",{value:e,enumerable:!0}):r,e)),Ee=e=>Q(B({},"__esModule",{value:!0}),e);var He={};we(He,{activate:()=>$e,deactivate:()=>Le});module.exports=Ee(He);var g=C(require("vscode"));var d=C(require("vscode")),le=C(require("path"));var M=C(require("vscode"));function X(e){let t=M.Uri.joinPath(e,".."),r=e.path.split("/").pop()?.replace(/\.nodegraph\.json$/,"")??"graph";return M.Uri.joinPath(t,`.${r}-imgs`)}function Ie(e,t,r){let n=M.Uri.joinPath(X(t),r);return e.asWebviewUri(n).toString()}var ee=/\[\[IMG:([^:\]]+)(?::[^\]]+)?\]\]/g;function G(e,t,r){let n={},o=a=>{a&&!n[a]&&(n[a]=Ie(e,t,a))};for(let a of r.nodes){ee.lastIndex=0;let l;for(;(l=ee.exec(a.content??""))!==null;)o(l[1])}for(let a of r.canvasImages??[])o(a.filename);return n}async function te(e,t,r,n="png"){let o=X(t);try{await M.workspace.fs.createDirectory(o)}catch{}let a=`img_${Date.now()}.${n}`,l=M.Uri.joinPath(o,a);return await M.workspace.fs.writeFile(l,Buffer.from(r,"base64")),{filename:a,webviewUri:e.asWebviewUri(l).toString()}}async function re(e,t){let r=M.Uri.joinPath(X(e),t);try{await M.workspace.fs.delete(r)}catch{}}var oe=C(require("path"));function $(e){let t=e.match(/^(.+):(\d+)(?:-(\d+))?$/);return t?{path:t[1],startLine:parseInt(t[2],10),endLine:t[3]?parseInt(t[3],10):void 0}:{path:e}}function y(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function ke(e){let t=e.trim().replace("#",""),r=t.length===3?t.split("").map(n=>n+n).join(""):t;return/^[0-9a-fA-F]{6}$/.test(r)?{r:255-parseInt(r.slice(0,2),16),g:255-parseInt(r.slice(2,4),16),b:255-parseInt(r.slice(4,6),16)}:null}var Se=e=>e.replace(/[^a-zA-Z0-9_-]/g,"_");function L(e){return/^\s*\|/.test(e)&&e.indexOf("|",1)!==-1}function z(e){return/^\s*\|[\s\-:|]+\|\s*$/.test(e)&&!/[a-zA-Z0-9]/.test(e)}function ne(e){return e.replace(/^\s*\|/,"").replace(/\|\s*$/,"").split("|").map(t=>t.trim())}function De(e){if(!e)return[{type:"text",text:"",startChar:0,endChar:0}];let t=e.split(`
`),r=[],n=0,o=0,a=l=>t[l].length+(l<t.length-1?1:0);for(;n<t.length;)if(L(t[n])&&n+1<t.length&&z(t[n+1])){let h=o,i=[];for(;n<t.length&&L(t[n]);)i.push(t[n]),o+=a(n),n++;i.length>=3?r.push({type:"table",headers:ne(i[0]),rows:i.slice(2).map(ne),startChar:h,endChar:o}):r.push({type:"text",text:i.join(`
...
`),startChar:h,endChar:o})}return r}function W(e){let t=e.split(`
`);for(let r=0;r+1<t.length;r++)if(L(t[r])&&z(t[r+1]))return!0;return!1}function Ae(e){return y(e).replace(/\\\$/g,()=>'<span class="ng-cur">$</span>')}function H(e){return Ae(e).replace(/\*\*(.+?)\*\*/g,'<strong style="font-size:1.1em">$1</strong>')}function F(e,t){let r=/\[\[IMG:([^:\]]+)(?::(\d+)x(\d+))?\]\]/g,n="",o=0,a;for(;(a=r.exec(e))!==null;){a.index>o&&(n+=H(e.slice(o,a.index)));let l=a[1],h=a[2],i=a[3],c=h&&i?` width="${h}" height="${i}"`:"",p=t[l];n+=p?`<img class="ng-img${c?" ng-img-sized":""}" src="${p}"${c} alt="${y(l)}" onclick="showLightbox(this.src)" title="Click to enlarge">`:`<span class="ng-img-missing">${y(l)}</span>`,o=a.index+a[0].length}return o<e.length&&(n+=H(e.slice(o))),n}function Me(e,t){let r=e.headers.map(o=>`<th>${F(o,t)}</th>`).join(""),n=e.rows.map(o=>`<tr>${o.map(a=>`<td>${F(a,t)}</td>`).join("")}</tr>`).join("");return`<div class="ng-table-wrap"><table class="ng-table"><thead><tr>${r}</tr></thead><tbody>${n}</tbody></table></div>`}function Te(e,t,r,n,o,a){let l=t?.color??"#888",h=t?.shape==="rounded"?"22px":"2px",i=y(t?.label??e.template),c=Math.round(e.position.x+r),p=Math.round(e.position.y+n),u="",s=e.content??"";if(W(s)){let f=De(s);u+='<div class="ng-content">';for(let D of f)D.type==="table"?u+=Me(D,o):D.text&&(u+=`<div class="ng-seg">${F(D.text,o).replace(/\n/g,"<br>")}</div>`);u+="</div>"}else s&&(u+=`<div class="ng-content">${F(s,o).replace(/\n/g,"<br>")}</div>`);if(e.original){let f=y(e.original.title??"Original"),D=e.originalExpanded?" open":"";u+=`<details class="ng-original"${D}><summary>${f}${e.original.location?` <span class="ng-loc">${y(e.original.location)}</span>`:""}</summary>
...
<div class="ng-toggle-body">${H(f.content).replace(/\n/g,"<br>")}</div></details>`;e.links.length&&(u+=`<div class="ng-links">${e.links.map(f=>{let D=f.type==="url"?"\u{1F517}":f.type==="pdf"?"\u{1F4C4}":f.type==="obsidian"?"\u{1F7E3}":f.type==="code"?"\u{1F4BB}":"\u2B21";if(f.type==="code"){let{path:ge,startLine:K,endLine:J}=$(f.target),Z=oe.posix.normalize(a.repoPrefix+ge);return`<a class="ng-link"${a.githubBase&&!Z.startsWith("..")?` href="${y(a.githubBase)}/${y(Z)}${K?`#L${K}${J?`-L${J}`:""}`:""}" target="_blank"`:""}>${D} ${y(f.label||f.target)}</a>`}return`<a class="ng-link"${f.type==="url"||f.type==="pdf"?` href="${y(f.target)}" target="_blank"`:""}>${D} ${y(f.label||f.target)}</a>`}).join("")}</div>`);let b=!!u,v=e.contentExpanded?"":' style="display:none"',I=e.children.length?` data-children="${e.children.join(",")}"`:"",k=W(s)?" ng-has-table":"",T=/\[\[IMG:[^:\]]+:(\d+)x\d+\]\]/g,S=0,m;for(;(m=T.exec(s))!==null;)S=Math.max(S,Number(m[1]));let O=S>0?W(s)?S+280:S+32:0,A=Math.max(e.nodeWidth??0,432,O),N=[A>432?`min-width:${A}px`:"",e.nodeHeight&&e.contentExpanded?`min-height:${e.nodeHeight}px`:""].filter(Boolean).join(";"),Y=e.nodeHeight?` data-min-h="${e.nodeHeight}"`:"";return`<div class="ng-node${k}" id="node-${y(e.id)}"${I}${Y} style="--color:${l};border-radius:${h};left:${c}px;top:${p}px${N?";"+N:""}">
  <div class="ng-header" onclick="onHeaderClick(this)" title="Click to select node">
    <span class="ng-tag" onmousedown="onNodeTagMousedown(event,this.closest('.ng-node'))" style="background:color-mix(in srgb,${l} 20%,transparent);color:${l}">${i}</span>
...
</div>`}function ie(e,t={},r={githubBase:null,repoPrefix:""}){let n=1/0,o=1/0;for(let s of e.nodes)n=Math.min(n,s.position.x),o=Math.min(o,s.position.y);isFinite(n)||(n=0,o=0);let a=-n+100,l=-o+100,h=e.nodes.map(s=>Te(s,e.nodeTemplates[s.template],a,l,t,r)).join(`
`),i=JSON.stringify(e.nodes.map(s=>({id:s.id,lx:Math.round(s.position.x+a),ly:Math.round(s.position.y+l),children:s.children??[],template:s.template,contentExpanded:s.contentExpanded,isMain:s.template==="main_topic",nodeHeight:s.nodeHeight??null,naturalY:Math.round((s.nodeNaturalY??s.position.y)+l),title:s.title,content:s.content??"",originalTitle:s.original?.title??"",originalText:s.original?.text??"",toggles:(s.toggleItems??[]).map(b=>({id:b.id,title:b.title,content:b.content}))}))),c=JSON.stringify(e.edges.map(s=>({source:s.source,target:s.target,type:s.type,label:s.label||""}))),p=JSON.stringify(Object.fromEntries(Object.entries(e.nodeTemplates).map(([s,b])=>[s,b.label]))),u=Object.entries(e.nodeTemplates).map(([s,b])=>{let v=ke(b.color),I=v?`rgb(${v.r},${v.g},${v.b})`:"#ff3b30",k=v?`rgba(${v.r},${v.g},${v.b},0.18)`:"rgba(255,59,48,0.18)";return`::highlight(ng-hit-${Se(s)}){color:${I};background-color:${k};text-decoration:underline}`}).join(`
`);return`<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
</html>`}var Ce={main_topic:{label:"Main topic",color:"#4B8BBE",icon:"file-text",shape:"sharp"},method:{label:"Method",color:"#5C9E6E",icon:"cpu",shape:"sharp"},result:{label:"Result",color:"#9B59B6",icon:"bar-chart-2",shape:"sharp"},claim:{label:"Claim",color:"#E74C3C",icon:"alert-circle",shape:"sharp"},question:{label:"Question",color:"#E5A835",icon:"help-circle",shape:"rounded"},gap:{label:"Gap / Idea",color:"#1ABC9C",icon:"lightbulb",shape:"rounded"},reference:{label:"Reference",color:"#95A5A6",icon:"book-open",shape:"rounded"},memo:{label:"Memo",color:"#BDC3C7",icon:"edit-3",shape:"rounded"}};function _(e="New Graph"){let t=new Date().toISOString();return{version:"1.0.0",title:e,created:t,modified:t,nodeTemplates:Ce,nodes:[],edges:[],viewport:{x:0,y:0,zoom:1}}}function U(){let e="",t="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";for(let r=0;r<32;r++)e+=t.charAt(Math.floor(Math.random()*t.length));return e}var w=C(require("vscode"));var j=class e{static{this.panels=new Map}static async openAndSearch(t,r,n,o){let a=r.toString(),l=e.panels.get(a);if(l){l.panel.reveal(w.ViewColumn.Beside,!0),l.ready?l.panel.webview.postMessage({type:"search",query:n,pageHint:o}):l.pending={query:n,pageHint:o};return}let h;try{h=await w.workspace.fs.readFile(r)}catch{w.window.showErrorMessage(`PDF\uB97C \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4: ${r.fsPath}`);return}let i=w.window.createWebviewPanel("nodegraph.pdfViewer",r.path.split("/").pop()??"PDF",{viewColumn:w.ViewColumn.Beside,preserveFocus:!1},{enableScripts:!0,retainContextWhenHidden:!0,localResourceRoots:[w.Uri.joinPath(t.extensionUri,"dist")]}),c={panel:i,ready:!1,pending:{query:n,pageHint:o}};e.panels.set(a,c),i.iconPath=w.Uri.joinPath(t.extensionUri,"resources","icon-hires.png"),i.webview.html=e._getHtml(t,i.webview);let p=Buffer.from(h).toString("base64");i.webview.onDidReceiveMessage(u=>{u.type==="ready"&&(c.ready=!0,i.webview.postMessage({type:"load",pdfData:p,query:c.pending?.query,pageHint:c.pending?.pageHint}),c.pending=null)}),i.onDidDispose(()=>{e.panels.delete(a)})}static _getHtml(t,r){let n=r.asWebviewUri(w.Uri.joinPath(t.extensionUri,"dist","pdfviewer.js")),o=r.asWebviewUri(w.Uri.joinPath(t.extensionUri,"dist","pdfviewer.css")),a=r.asWebviewUri(w.Uri.joinPath(t.extensionUri,"dist","pdf.worker.min.mjs")),l=U();return`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
<body>
  ...
  <script nonce="${l}">window.__PDF_WORKER_URI__ = "${a}";</script>
  <script nonce="${l}" type="module" src="${n}"></script>
</body>
</html>`}};var ae=C(require("child_process"));function q(e,t){try{return ae.execSync(e,{cwd:t,timeout:5e3,stdio:["pipe","pipe","pipe"]}).toString().trim()}catch{return""}}function Pe(e){let t=e.match(/github\.com[:/]([^/]+)\/(.+?)(?:\.git)?$/);return t?`${t[1]}/${t[2]}`:null}function se(e){let t=q("git remote get-url origin",e);if(!t)return null;let r=Pe(t);if(!r)return null;let n=q("git rev-parse HEAD",e);return n?`https://github.com/${r}/blob/${n}`:null}function de(e){let t=q("git rev-parse --show-toplevel",e);if(!t)return"";let r=e.replace(/\\/g,"/").replace(t.replace(/\\/g,"/"),"").replace(/^\/+/,"");return r?`${r}/`:""}var P=class e{constructor(t){this.context=t;this._pendingSaves=new Set}static register(t){let r=new e(t);return d.window.registerCustomEditorProvider("nodegraph.editor",r,{webviewOptions:{retainContextWhenHidden:!0}})}static{this._activeWebview=null}static postToActive(t){e._activeWebview?.postMessage(t)}async resolveCustomTextEditor(t,r,n){r.iconPath=d.Uri.joinPath(this.context.extensionUri,"resources","icon-hires.png");let o=d.Uri.joinPath(t.uri,"..");r.webview.options={enableScripts:!0,localResourceRoots:[this.context.extensionUri,o]},r.webview.html=this._getHtmlForWebview(r.webview);let a=i=>{let c=t.getText();try{let p=c.trim()===""?_():JSON.parse(c),u=G(r.webview,t.uri,p);r.webview.postMessage({type:i,data:p,imageUris:u})}catch{}},l=r.webview.onDidReceiveMessage(async i=>{if(i.type==="ready")a("load");else if(i.type==="save"){let c=t.uri.toString();this._pendingSaves.add(c);try{let p=new d.WorkspaceEdit,u=new d.Range(t.positionAt(0),t.positionAt(t.getText().length));p.replace(t.uri,u,JSON.stringify(i.data,null,2)),await d.workspace.applyEdit(p),await t.save()}finally{this._pendingSaves.delete(c)}}else if(i.type==="openLink"){let c=i.link;if(c.type==="url")d.env.openExternal(d.Uri.parse(c.target));else if(c.type==="pdf"){let p=d.Uri.joinPath(d.Uri.joinPath(t.uri,".."),c.target);d.env.openExternal(p)}else if(c.type==="obsidian")d.env.openExternal(d.Uri.parse(c.target));else if(c.type==="code"){let{path:p,startLine:u,endLine:s}=$(c.target);try{let b=d.Uri.joinPath(d.Uri.joinPath(t.uri,".."),p),v=await d.workspace.openTextDocument(b),I=await d.window.showTextDocument(v,{preview:!1});if(u){let k=Math.max(0,u-1),T=Math.max(k,(s??u)-1),S=v.lineAt(Math.min(T,v.lineCount-1)).text.length,m=new d.Range(k,0,T,S);I.selection=new d.Selection(m.start,m.end),I.revealRange(m,d.TextEditorRevealType.InCenter)}}catch{d.window.showErrorMessage(`NodeGraph: couldn't open ${p}`)}}}else if(i.type==="searchInPdf"){let c=d.Uri.joinPath(d.Uri.joinPath(t.uri,".."),i.pdfTarget);j.openAndSearch(this.context,c,i.query,i.pageHint)}else if(i.type==="exportHtml")try{let c=i.data,p=d.Uri.joinPath(t.uri,".."),u=le.basename(t.uri.fsPath,".nodegraph.json"),s=d.Uri.joinPath(p,`.${u}-imgs`),b={},v=/\[\[IMG:([^:\]]+)(?::[^\]]+)?\]\]/g,I=async A=>{if(!(!A||b[A]))try{let N=d.Uri.joinPath(s,A),Y=await d.workspace.fs.readFile(N),f=A.split(".").pop()?.toLowerCase()??"png",D=f==="jpg"||f==="jpeg"?"image/jpeg":f==="gif"?"image/gif":f==="webp"?"image/webp":"image/png";b[A]=`data:${D};base64,${Buffer.from(Y).toString("base64")}`}catch{}};for(let A of c.nodes){v.lastIndex=0;let N;for(;(N=v.exec(A.content??""))!==null;)await I(N[1])}let k=se(p.fsPath),T=k?de(p.fsPath):"",S=ie(c,b,{githubBase:k,repoPrefix:T}),m=d.Uri.joinPath(p,`${u}.html`);await d.workspace.fs.writeFile(m,Buffer.from(S,"utf-8"));let O=await d.window.showInformationMessage(`HTML exported: ${u}.html`,"Open in Browser","Show in Explorer");O==="Open in Browser"?d.env.openExternal(m):O==="Show in Explorer"&&d.commands.executeCommand("revealFileInOS",m)}catch(c){d.window.showErrorMessage(`HTML export failed: ${c}`)}else if(i.type==="saveImage")try{let{filename:c,webviewUri:p}=await te(r.webview,t.uri,i.data,i.ext??"png");r.webview.postMessage({type:"imageSaved",nodeId:i.nodeId,filename:c,webviewUri:p})}catch(c){d.window.showErrorMessage(`Failed to save image: ${c}`)}else if(i.type==="deleteImageFile")await re(t.uri,i.filename);else if(i.type==="reload")try{let c=await d.workspace.fs.readFile(t.uri),p=Buffer.from(c).toString("utf-8"),u=JSON.parse(p),s=G(r.webview,t.uri,u);r.webview.postMessage({type:"load",data:u,imageUris:s})}catch{a("load")}else if(i.type==="openHelp"){let c=d.Uri.joinPath(this.context.extensionUri,"README.md");d.commands.executeCommand("markdown.showPreviewToSide",c.with({fragment:"features"}))}}),h=d.workspace.onDidChangeTextDocument(i=>{i.document.uri.toString()===t.uri.toString()&&(this._pendingSaves.has(t.uri.toString())||a("externalChange"))});e._activeWebview=r.webview,r.onDidChangeViewState(i=>{i.webviewPanel.active&&(e._activeWebview=r.webview,r.webview.postMessage({type:"focusCanvas"}))}),r.onDidDispose(()=>{l.dispose(),h.dispose(),e._activeWebview===r.webview&&(e._activeWebview=null)})}_getHtmlForWebview(t){let r=t.asWebviewUri(d.Uri.joinPath(this.context.extensionUri,"dist","webview.js")),n=t.asWebviewUri(d.Uri.joinPath(this.context.extensionUri,"dist","katex","katex.min.css")),o=U();return`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
