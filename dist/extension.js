"use strict";var ve=Object.create;var B=Object.defineProperty;var xe=Object.getOwnPropertyDescriptor;var be=Object.getOwnPropertyNames;var ye=Object.getPrototypeOf,we=Object.prototype.hasOwnProperty;var Ee=(e,t)=>{for(var r in t)B(e,r,{get:t[r],enumerable:!0})},Q=(e,t,r,n)=>{if(t&&typeof t=="object"||typeof t=="function")for(let o of be(t))!we.call(e,o)&&o!==r&&B(e,o,{get:()=>t[o],enumerable:!(n=xe(t,o))||n.enumerable});return e};var M=(e,t,r)=>(r=e!=null?ve(ye(e)):{},Q(t||!e||!e.__esModule?B(r,"default",{value:e,enumerable:!0}):r,e)),Ie=e=>Q(B({},"__esModule",{value:!0}),e);var He={};Ee(He,{activate:()=>Le,deactivate:()=>Fe});module.exports=Ie(He);var g=M(require("vscode"));var d=M(require("vscode")),ce=M(require("path"));var C=M(require("vscode"));function Y(e){let t=C.Uri.joinPath(e,".."),r=e.path.split("/").pop()?.replace(/\.nodegraph\.json$/,"")??"graph";return C.Uri.joinPath(t,`.${r}-imgs`)}function ke(e,t,r){let n=C.Uri.joinPath(Y(t),r);return e.asWebviewUri(n).toString()}var ee=/\[\[IMG:([^:\]]+)(?::[^\]]+)?\]\]/g;function X(e,t,r){let n={},o=a=>{a&&!n[a]&&(n[a]=ke(e,t,a))};for(let a of r.nodes){ee.lastIndex=0;let c;for(;(c=ee.exec(a.content??""))!==null;)o(c[1])}for(let a of r.canvasImages??[])o(a.filename);return n}async function te(e,t,r,n="png"){let o=Y(t);try{await C.workspace.fs.createDirectory(o)}catch{}let a=`img_${Date.now()}.${n}`,c=C.Uri.joinPath(o,a);return await C.workspace.fs.writeFile(c,Buffer.from(r,"base64")),{filename:a,webviewUri:e.asWebviewUri(c).toString()}}async function re(e,t){let r=C.Uri.joinPath(Y(e),t);try{await C.workspace.fs.delete(r)}catch{}}var oe=M(require("path"));function $(e){let t=e.match(/^(.+):(\d+)(?:-(\d+))?$/);return t?{path:t[1],startLine:parseInt(t[2],10),endLine:t[3]?parseInt(t[3],10):void 0}:{path:e}}function y(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function Se(e){let t=e.trim().replace("#",""),r=t.length===3?t.split("").map(n=>n+n).join(""):t;return/^[0-9a-fA-F]{6}$/.test(r)?{r:255-parseInt(r.slice(0,2),16),g:255-parseInt(r.slice(2,4),16),b:255-parseInt(r.slice(4,6),16)}:null}var De=e=>e.replace(/[^a-zA-Z0-9_-]/g,"_");function L(e){return/^\s*\|/.test(e)&&e.indexOf("|",1)!==-1}function z(e){return/^\s*\|[\s\-:|]+\|\s*$/.test(e)&&!/[a-zA-Z0-9]/.test(e)}function ne(e){return e.replace(/^\s*\|/,"").replace(/\|\s*$/,"").split("|").map(t=>t.trim())}function Ae(e){if(!e)return[{type:"text",text:"",startChar:0,endChar:0}];let t=e.split(`
`),r=[],n=0,o=0,a=c=>t[c].length+(c<t.length-1?1:0);for(;n<t.length;)if(L(t[n])&&n+1<t.length&&z(t[n+1])){let h=o,i=[];for(;n<t.length&&L(t[n]);)i.push(t[n]),o+=a(n),n++;i.length>=3?r.push({type:"table",headers:ne(i[0]),rows:i.slice(2).map(ne),startChar:h,endChar:o}):r.push({type:"text",text:i.join(`
`),startChar:h,endChar:o})}else{let h=o,i=[];for(;n<t.length&&!(L(t[n])&&n+1<t.length&&z(t[n+1]));)i.push(t[n]),o+=a(n),n++;r.push({type:"text",text:i.join(`
`),startChar:h,endChar:o})}return r}function G(e){let t=e.split(`
`);for(let r=0;r+1<t.length;r++)if(L(t[r])&&z(t[r+1]))return!0;return!1}function Me(e){return y(e).replace(/\\\$/g,()=>'<span class="ng-cur">$</span>')}function F(e){return Me(e).replace(/\*\*(.+?)\*\*/g,'<strong style="font-size:1.1em">$1</strong>')}function H(e,t){let r=/\[\[IMG:([^:\]]+)(?::(\d+)x(\d+))?\]\]/g,n="",o=0,a;for(;(a=r.exec(e))!==null;){a.index>o&&(n+=F(e.slice(o,a.index)));let c=a[1],h=a[2],i=a[3],l=h&&i?` width="${h}" height="${i}"`:"",p=t[c];n+=p?`<img class="ng-img${l?" ng-img-sized":""}" src="${p}"${l} alt="${y(c)}" onclick="showLightbox(this.src)" title="Click to enlarge">`:`<span class="ng-img-missing">${y(c)}</span>`,o=a.index+a[0].length}return o<e.length&&(n+=F(e.slice(o))),n}function Ce(e,t){let r=e.headers.map(o=>`<th>${H(o,t)}</th>`).join(""),n=e.rows.map(o=>`<tr>${o.map(a=>`<td>${H(a,t)}</td>`).join("")}</tr>`).join("");return`<div class="ng-table-wrap"><table class="ng-table"><thead><tr>${r}</tr></thead><tbody>${n}</tbody></table></div>`}function Te(e,t,r,n,o,a){let c=t?.color??"#888",h=t?.shape==="rounded"?"22px":"2px",i=y(t?.label??e.template),l=Math.round(e.position.x+r),p=Math.round(e.position.y+n),u="",s=e.content??"";if(G(s)){let f=Ae(s);u+='<div class="ng-content">';for(let D of f)D.type==="table"?u+=Ce(D,o):D.text&&(u+=`<div class="ng-seg">${H(D.text,o).replace(/\n/g,"<br>")}</div>`);u+="</div>"}else s&&(u+=`<div class="ng-content">${H(s,o).replace(/\n/g,"<br>")}</div>`);if(e.original){let f=y(e.original.title??"Original"),D=e.originalExpanded?" open":"";u+=`<details class="ng-original"${D}><summary>${f}${e.original.location?` <span class="ng-loc">${y(e.original.location)}</span>`:""}</summary>
<div class="ng-orig-text">${F(e.original.text).replace(/\n/g,"<br>")}</div></details>`}for(let f of e.toggleItems??[])u+=`<details class="ng-toggle" data-toggle-id="${y(f.id)}"${f.expanded?" open":""}><summary>${y(f.title||"(untitled)")}</summary>
<div class="ng-toggle-body">${F(f.content).replace(/\n/g,"<br>")}</div></details>`;e.links.length&&(u+=`<div class="ng-links">${e.links.map(f=>{let D=f.type==="url"?"\u{1F517}":f.type==="pdf"?"\u{1F4C4}":f.type==="obsidian"?"\u{1F7E3}":f.type==="code"?"\u{1F4BB}":"\u2B21";if(f.type==="code"){let{path:me,startLine:K,endLine:J}=$(f.target),Z=oe.posix.normalize(a.repoPrefix+me);return`<a class="ng-link"${a.githubBase&&!Z.startsWith("..")?` href="${y(a.githubBase)}/${y(Z)}${K?`#L${K}${J?`-L${J}`:""}`:""}" target="_blank"`:""}>${D} ${y(f.label||f.target)}</a>`}return`<a class="ng-link"${f.type==="url"||f.type==="pdf"?` href="${y(f.target)}" target="_blank"`:""}>${D} ${y(f.label||f.target)}</a>`}).join("")}</div>`);let b=!!u,v=e.contentExpanded?"":' style="display:none"',I=e.children.length?` data-children="${e.children.join(",")}"`:"",k=G(s)?" ng-has-table":"",T=/\[\[IMG:[^:\]]+:(\d+)x\d+\]\]/g,S=0,m;for(;(m=T.exec(s))!==null;)S=Math.max(S,Number(m[1]));let O=S>0?G(s)?S+280:S+32:0,A=Math.max(e.nodeWidth??0,432,O),N=[A>432?`min-width:${A}px`:"",e.nodeHeight&&e.contentExpanded?`min-height:${e.nodeHeight}px`:""].filter(Boolean).join(";"),W=e.nodeHeight?` data-min-h="${e.nodeHeight}"`:"";return`<div class="ng-node${k}" id="node-${y(e.id)}"${I}${W} style="--color:${c};border-radius:${h};left:${l}px;top:${p}px${N?";"+N:""}">
  <div class="ng-header" onclick="onHeaderClick(this)" title="Click to select node">
    <span class="ng-tag" onmousedown="onNodeTagMousedown(event,this.closest('.ng-node'))" style="background:color-mix(in srgb,${c} 20%,transparent);color:${c}">${i}</span>
    ${b?`<span class="ng-title" onclick="onTitleClick(event,this)" title="Click to fold/unfold">${y(e.title)}</span>`:`<span class="ng-title">${y(e.title)}</span>`}
  </div>
  ${b?`<div class="ng-body"${v}${e.fontSize?` style="font-size:${e.fontSize}px"`:""}>${u}</div>`:""}
</div>`}function ie(e,t={},r={githubBase:null,repoPrefix:""}){let n=1/0,o=1/0;for(let s of e.nodes)n=Math.min(n,s.position.x),o=Math.min(o,s.position.y);isFinite(n)||(n=0,o=0);let a=-n+100,c=-o+100,h=e.nodes.map(s=>Te(s,e.nodeTemplates[s.template],a,c,t,r)).join(`
`),i=JSON.stringify(e.nodes.map(s=>({id:s.id,lx:Math.round(s.position.x+a),ly:Math.round(s.position.y+c),children:s.children??[],template:s.template,contentExpanded:s.contentExpanded,isMain:s.template==="main_topic",nodeHeight:s.nodeHeight??null,naturalY:Math.round((s.nodeNaturalY??s.position.y)+c),title:s.title,content:s.content??"",originalTitle:s.original?.title??"",originalText:s.original?.text??"",toggles:(s.toggleItems??[]).map(b=>({id:b.id,title:b.title,content:b.content}))}))),l=JSON.stringify(e.edges.map(s=>({source:s.source,target:s.target,type:s.type,label:s.label||""}))),p=JSON.stringify(Object.fromEntries(Object.entries(e.nodeTemplates).map(([s,b])=>[s,b.label]))),u=Object.entries(e.nodeTemplates).map(([s,b])=>{let v=Se(b.color),I=v?`rgb(${v.r},${v.g},${v.b})`:"#ff3b30",k=v?`rgba(${v.r},${v.g},${v.b},0.18)`:"rgba(255,59,48,0.18)";return`::highlight(ng-hit-${De(s)}){color:${I};background-color:${k};text-decoration:underline}`}).join(`
`);return`<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>${y(e.title)}</title>
...
<div id="toolbar">
  <div id="tb-row1">
    <span id="tb-title">${y(e.title)}</span>
  </div>
  <div id="tb-row2">
...
window.addEventListener('load', function() {
  ...
  imgs.forEach(function(img) {
...
</html>`}var Pe={main_topic:{label:"Main topic",color:"#4B8BBE",icon:"file-text",shape:"sharp"},method:{label:"Method",color:"#5C9E6E",icon:"cpu",shape:"sharp"},result:{label:"Result",color:"#9B59B6",icon:"bar-chart-2",shape:"sharp"},claim:{label:"Claim",color:"#E74C3C",icon:"alert-circle",shape:"sharp"},question:{label:"Question",color:"#E5A835",icon:"help-circle",shape:"rounded"},gap:{label:"Gap / Idea",color:"#1ABC9C",icon:"lightbulb",shape:"rounded"},reference:{label:"Reference",color:"#95A5A6",icon:"book-open",shape:"rounded"},memo:{label:"Memo",color:"#BDC3C7",icon:"edit-3",shape:"rounded"}};function _(e="New Graph"){let t=new Date().toISOString();return{version:"1.0.0",title:e,created:t,modified:t,nodeTemplates:Pe,nodes:[],edges:[],viewport:{x:0,y:0,zoom:1}}}function U(){let e="",t="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";for(let r=0;r<32;r++)e+=t.charAt(Math.floor(Math.random()*t.length));return e}var w=M(require("vscode")),ae=M(require("fs"));var j=class e{static{this.panels=new Map}static async openAndSearch(t,r,n,o){let a=r.toString(),c=e.panels.get(a);if(c){c.panel.reveal(w.ViewColumn.Beside,!0),c.ready?c.panel.webview.postMessage({type:"search",query:n,pageHint:o}):c.pending={query:n,pageHint:o};return}let h;try{h=await w.workspace.fs.readFile(r)}catch{w.window.showErrorMessage(`PDF\uB97C \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4: ${r.fsPath}`);return}let i=w.window.createWebviewPanel("nodegraph.pdfViewer",r.path.split("/").pop()??"PDF",{viewColumn:w.ViewColumn.Beside,preserveFocus:!1},{enableScripts:!0,retainContextWhenHidden:!0,localResourceRoots:[w.Uri.joinPath(t.extensionUri,"dist")]}),l={panel:i,ready:!1,pending:{query:n,pageHint:o}};e.panels.set(a,l),i.iconPath=w.Uri.joinPath(t.extensionUri,"resources","icon-hires.png"),i.webview.html=e._getHtml(t,i.webview);let p=Buffer.from(h).toString("base64");i.webview.onDidReceiveMessage(u=>{u.type==="ready"&&(l.ready=!0,i.webview.postMessage({type:"load",pdfData:p,query:l.pending?.query,pageHint:l.pending?.pageHint}),l.pending=null)}),i.onDidDispose(()=>{e.panels.delete(a)})}static _getHtml(t,r){let n=w.Uri.joinPath(t.extensionUri,"dist","pdfjs-viewer","web"),o=r.asWebviewUri(n).toString()+"/",a=r.asWebviewUri(w.Uri.joinPath(t.extensionUri,"dist","pdfjs-viewer","build","pdf.worker.min.mjs")),c=w.Uri.joinPath(n,"viewer.html").fsPath,h=U(),i=`default-src 'none'; img-src ${r.cspSource} data: blob:; script-src 'nonce-${h}' ${r.cspSource}; style-src 'unsafe-inline' ${r.cspSource}; worker-src ${r.cspSource} blob:; connect-src ${r.cspSource} blob:;`;return ae.readFileSync(c,"utf-8").replace('<meta charset="utf-8">',`<meta charset="utf-8">
    ...
    </script>`)}};var se=M(require("child_process"));function q(e,t){try{return se.execSync(e,{cwd:t,timeout:5e3,stdio:["pipe","pipe","pipe"]}).toString().trim()}catch{return""}}function Ne(e){let t=e.match(/github\.com[:/]([^/]+)\/(.+?)(?:\.git)?$/);return t?`${t[1]}/${t[2]}`:null}function de(e){let t=q("git remote get-url origin",e);if(!t)return null;let r=Ne(t);if(!r)return null;let n=q("git rev-parse HEAD",e);return n?`https://github.com/${r}/blob/${n}`:null}function le(e){let t=q("git rev-parse --show-toplevel",e);if(!t)return"";let r=e.replace(/\\/g,"/").replace(t.replace(/\\/g,"/"),"").replace(/^\/+/,"");return r?`${r}/`:""}var P=class e{constructor(t){this.context=t;this._pendingSaves=new Set}static register(t){let r=new e(t);return d.window.registerCustomEditorProvider("nodegraph.editor",r,{webviewOptions:{retainContextWhenHidden:!0}})}static{this._activeWebview=null}static postToActive(t){e._activeWebview?.postMessage(t)}async resolveCustomTextEditor(t,r,n){r.iconPath=d.Uri.joinPath(this.context.extensionUri,"resources","icon-hires.png");let o=d.Uri.joinPath(t.uri,"..");r.webview.options={enableScripts:!0,localResourceRoots:[this.context.extensionUri,o]},r.webview.html=this._getHtmlForWebview(r.webview);let a=i=>{let l=t.getText();try{let p=l.trim()===""?_():JSON.parse(l),u=X(r.webview,t.uri,p);r.webview.postMessage({type:i,data:p,imageUris:u})}catch{}},c=r.webview.onDidReceiveMessage(async i=>{if(i.type==="ready")a("load");else if(i.type==="save"){let l=t.uri.toString();this._pendingSaves.add(l);try{let p=new d.WorkspaceEdit,u=new d.Range(t.positionAt(0),t.positionAt(t.getText().length));p.replace(t.uri,u,JSON.stringify(i.data,null,2)),await d.workspace.applyEdit(p),await t.save()}finally{this._pendingSaves.delete(l)}}else if(i.type==="openLink"){let l=i.link;if(l.type==="url")d.env.openExternal(d.Uri.parse(l.target));else if(l.type==="pdf"){let p=d.Uri.joinPath(d.Uri.joinPath(t.uri,".."),l.target);d.env.openExternal(p)}else if(l.type==="obsidian")d.env.openExternal(d.Uri.parse(l.target));else if(l.type==="code"){let{path:p,startLine:u,endLine:s}=$(l.target);try{let b=d.Uri.joinPath(d.Uri.joinPath(t.uri,".."),p),v=await d.workspace.openTextDocument(b),I=await d.window.showTextDocument(v,{preview:!1});if(u){let k=Math.max(0,u-1),T=Math.max(k,(s??u)-1),S=v.lineAt(Math.min(T,v.lineCount-1)).text.length,m=new d.Range(k,0,T,S);I.selection=new d.Selection(m.start,m.end),I.revealRange(m,d.TextEditorRevealType.InCenter)}}catch{d.window.showErrorMessage(`NodeGraph: couldn't open ${p}`)}}}else if(i.type==="searchInPdf"){let l=d.Uri.joinPath(d.Uri.joinPath(t.uri,".."),i.pdfTarget);j.openAndSearch(this.context,l,i.query,i.pageHint)}else if(i.type==="exportHtml")try{let l=i.data,p=d.Uri.joinPath(t.uri,".."),u=ce.basename(t.uri.fsPath,".nodegraph.json"),s=d.Uri.joinPath(p,`.${u}-imgs`),b={},v=/\[\[IMG:([^:\]]+)(?::[^\]]+)?\]\]/g,I=async A=>{if(!(!A||b[A]))try{let N=d.Uri.joinPath(s,A),W=await d.workspace.fs.readFile(N),f=A.split(".").pop()?.toLowerCase()??"png",D=f==="jpg"||f==="jpeg"?"image/jpeg":f==="gif"?"image/gif":f==="webp"?"image/webp":"image/png";b[A]=`data:${D};base64,${Buffer.from(W).toString("base64")}`}catch{}};for(let A of l.nodes){v.lastIndex=0;let N;for(;(N=v.exec(A.content??""))!==null;)await I(N[1])}let k=de(p.fsPath),T=k?le(p.fsPath):"",S=ie(l,b,{githubBase:k,repoPrefix:T}),m=d.Uri.joinPath(p,`${u}.html`);await d.workspace.fs.writeFile(m,Buffer.from(S,"utf-8"));let O=await d.window.showInformationMessage(`HTML exported: ${u}.html`,"Open in Browser","Show in Explorer");O==="Open in Browser"?d.env.openExternal(m):O==="Show in Explorer"&&d.commands.executeCommand("revealFileInOS",m)}catch(l){d.window.showErrorMessage(`HTML export failed: ${l}`)}else if(i.type==="saveImage")try{let{filename:l,webviewUri:p}=await te(r.webview,t.uri,i.data,i.ext??"png");r.webview.postMessage({type:"imageSaved",nodeId:i.nodeId,filename:l,webviewUri:p})}catch(l){d.window.showErrorMessage(`Failed to save image: ${l}`)}else if(i.type==="deleteImageFile")await re(t.uri,i.filename);else if(i.type==="reload")try{let l=await d.workspace.fs.readFile(t.uri),p=Buffer.from(l).toString("utf-8"),u=JSON.parse(p),s=X(r.webview,t.uri,u);r.webview.postMessage({type:"load",data:u,imageUris:s})}catch{a("load")}else if(i.type==="openHelp"){let l=d.Uri.joinPath(this.context.extensionUri,"README.md");d.commands.executeCommand("markdown.showPreviewToSide",l.with({fragment:"features"}))}}),h=d.workspace.onDidChangeTextDocument(i=>{i.document.uri.toString()===t.uri.toString()&&(this._pendingSaves.has(t.uri.toString())||a("externalChange"))});e._activeWebview=r.webview,r.onDidChangeViewState(i=>{i.webviewPanel.active&&(e._activeWebview=r.webview,r.webview.postMessage({type:"focusCanvas"}))}),r.onDidDispose(()=>{c.dispose(),h.dispose(),e._activeWebview===r.webview&&(e._activeWebview=null)})}_getHtmlForWebview(t){let r=t.asWebviewUri(d.Uri.joinPath(this.context.extensionUri,"dist","webview.js")),n=t.asWebviewUri(d.Uri.joinPath(this.context.extensionUri,"dist","katex","katex.min.css")),o=U();return`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
</html>`}};var x=M(require("vscode")),pe=M(require("child_process"));function R(e){try{return pe.execSync(e,{timeout:5e3,stdio:["pipe","pipe","pipe"]}).toString().trim()}catch{return""}}function E(e){return R(e)!==""}function Oe(){let e=[],t=new Date().toISOString(),r=process.platform,n=r==="win32"?"Windows":r==="darwin"?"macOS":"Linux",o=process.arch,a=R("python3 --version 2>&1")||R("python --version 2>&1"),c=E("python3 --version 2>&1")?"python3":E("python --version 2>&1")?"python":"",h=c!=="",i=h&&E(`${c} -c "import fitz" 2>&1 && echo ok`),l=i?R(`${c} -c "import fitz; print(fitz.__version__)"`):"",p=h&&E(`${c} -c "import pdfplumber" 2>&1 && echo ok`),u=h&&E(`${c} -c "import pdfminer" 2>&1 && echo ok`),s=h&&E(`${c} -c "from PIL import Image" 2>&1 && echo ok`),b=s?R(`${c} -c "from PIL import __version__; print(__version__)"`):"",v=h&&E(`${c} -c "import cv2" 2>&1 && echo ok`),I=E("pdftotext -v 2>&1 && echo ok")||E("pdftotext --help 2>&1 && echo ok"),k=E("convert --version 2>&1 && echo ok"),T=E("magick --version 2>&1 && echo ok"),S=E("gs --version 2>&1 && echo ok")||E("gswin64c --version 2>&1 && echo ok"),m=O=>O?"\u2705":"\u274C";return e.push("# NodeGraph \u2014 Agent Environment Report"),e.push(""),e.push("> Auto-generated by the NodeGraph extension at activation."),e.push("> **AI agents: read this file to understand what tools are available on this machine.**"),e.push("> Re-generated each time a `.nodegraph.json` file is opened."),e.push(""),e.push(`Generated: \`${t}\``),e.push(""),e.push("---"),e.push(""),e.push("## System"),e.push(""),e.push("| | |"),e.push("|---|---|"),e.push(`| OS | ${n} (\`${r}\`) |`),e.push(`| Architecture | \`${o}\` |`),e.push(`| Python | ${h?`${m(!0)} \`${a}\``:`${m(!1)} not found`} |`),e.push(`| Python command | ${h?`\`${c}\``:"N/A"} |`),e.push(""),e.push("---"),e.push(""),e.push("## PDF Reading Capabilities"),e.push(""),e.push("| Tool | Available | Notes |"),e.push("|------|:---------:|-------|"),e.push(`| PyMuPDF (\`fitz\`) | ${m(i)} | ${i?`v${l} \u2014 recommended`:"Install: `pip install pymupdf`"} |`),e.push(`| pdfplumber | ${m(p)} | ${p?"available":"Install: `pip install pdfplumber`"} |`),e.push(`| pdfminer | ${m(u)} | ${u?"available":"Install: `pip install pdfminer.six`"} |`),e.push(`| poppler (\`pdftotext\`) | ${m(I)} | ${I?"CLI tool available":r==="win32"?"Install: download poppler for Windows":r==="darwin"?"Install: `brew install poppler`":"Install: `apt install poppler-utils`"} |`),e.push(`| Ghostscript (\`gs\`) | ${m(S)} | ${S?"available":"optional"} |`),e.push(""),e.push("---"),e.push(""),e.push("## Image Processing Capabilities"),e.push(""),e.push("| Tool | Available | Notes |"),e.push("|------|:---------:|-------|"),e.push(`| Pillow (\`PIL\`) | ${m(s)} | ${s?`v${b} \u2014 recommended`:"Install: `pip install Pillow`"} |`),e.push(`| OpenCV (\`cv2\`) | ${m(v)} | ${v?"available":"Install: `pip install opencv-python`"} |`),e.push(`| ImageMagick (\`convert\`) | ${m(k||T)} | ${k||T?"CLI tool available":r==="win32"?"Install: imagemagick.org":r==="darwin"?"Install: `brew install imagemagick`":"Install: `apt install imagemagick`"} |`),e.push(""),e.push("---"),e.push(""),e.push("## Agent Recommendations"),e.push(""),h||(e.push("> \u26A0\uFE0F **Python not found.** PDF reading and image processing via Python are not available."),e.push("> Install Python from https://python.org, then reopen a `.nodegraph.json` file to re-run this check."),e.push("")),e.push("### Reading a PDF"),i?(e.push("Use PyMuPDF (recommended \u2014 fastest and most accurate):"),e.push("```python"),e.push("import fitz"),e.push('doc = fitz.open("paper.pdf")'),e.push('text = "\\n".join(page.get_text() for page in doc)'),e.push("```")):p?(e.push("Use pdfplumber:"),e.push("```python"),e.push("import pdfplumber"),e.push('with pdfplumber.open("paper.pdf") as pdf:'),e.push('    text = "\\n".join(p.extract_text() or "" for p in pdf.pages)'),e.push("```")):I?(e.push("Use poppler CLI:"),e.push("```bash"),e.push("pdftotext paper.pdf -"),e.push("```")):e.push("\u274C No PDF reading tool available. Ask the user to install PyMuPDF: `pip install pymupdf`"),e.push(""),e.push("### Extracting images from a PDF"),i?(e.push("```python"),e.push("import fitz"),e.push('doc = fitz.open("paper.pdf")'),e.push("for i, page in enumerate(doc):"),e.push("    for img in page.get_images():"),e.push("        xref = img[0]"),e.push("        pix = fitz.Pixmap(doc, xref)"),e.push('        pix.save(f"fig_{i}_{xref}.png")'),e.push("```")):s?e.push("Pillow is available but cannot extract from PDF directly. Use PyMuPDF for extraction."):e.push("\u274C No image extraction tool available."),e.push(""),e.push("---"),e.push(""),e.push("*To refresh this report, reopen any `.nodegraph.json` file.*"),e.join(`
`)}async function V(e){let t=x.Uri.joinPath(e,".agent"),r=x.Uri.joinPath(t,"ENVIRONMENT.md");try{return await x.workspace.fs.createDirectory(t),await x.workspace.fs.writeFile(r,Buffer.from(Oe(),"utf-8")),!0}catch{return!1}}async function ue(e){if(!(!e||e.length===0))for(let t of e)await V(t.uri)}async function he(e,t){let r=x.Uri.joinPath(e,".agent","NODEGRAPH_SPEC.md"),n;try{n=await x.workspace.fs.readFile(r)}catch{return!1}let o=x.Uri.joinPath(t,".agent"),a=x.Uri.joinPath(o,"NODEGRAPH_SPEC.md");try{return await x.workspace.fs.createDirectory(o),await x.workspace.fs.writeFile(a,n),!0}catch{return!1}}async function fe(e,t){let r=x.Uri.joinPath(e,".prompt"),n=x.Uri.joinPath(t,".prompt");try{for(let o of["paper","lecture","code"]){let a=x.Uri.joinPath(n,o);await x.workspace.fs.createDirectory(a);for(let c of["korean.md","english.md"]){let h=await x.workspace.fs.readFile(x.Uri.joinPath(r,o,c));await x.workspace.fs.writeFile(x.Uri.joinPath(a,c),h)}}return!0}catch{return!1}}var Re=[{id:"tomoki1207.pdf",name:"vscode-pdf (PDF Viewer)"}];async function Be(){for(let e of Re)if(!g.extensions.getExtension(e.id))try{await g.commands.executeCommand("workbench.extensions.installExtension",e.id)}catch{}}async function ge(e){if(e)return e;let t=g.workspace.workspaceFolders??[];return t.length===0?void 0:t.length===1?t[0].uri:(await g.window.showWorkspaceFolderPick({placeHolder:"Select a folder for NodeGraph"}))?.uri}async function $e(e){let t=await ge(e),r=t?g.Uri.joinPath(t,"untitled.nodegraph.json"):void 0,n=await g.window.showSaveDialog({defaultUri:r,filters:{NodeGraph:["nodegraph.json"]},title:"Create New NodeGraph"});if(!n)return;let o=n.fsPath.endsWith(".nodegraph.json")?n:n.with({path:n.path.replace(/(\.nodegraph)?(\.json)?$/,"")+".nodegraph.json"}),a=_();await g.workspace.fs.writeFile(o,Buffer.from(JSON.stringify(a,null,2),"utf-8")),await g.commands.executeCommand("vscode.openWith",o,"nodegraph.editor")}function Le(e){e.subscriptions.push(P.register(e)),e.subscriptions.push(g.commands.registerCommand("nodegraph.search",()=>{P.postToActive({type:"openSearch"})}),g.commands.registerCommand("nodegraph.fitView",()=>{P.postToActive({type:"fitView"})}),g.commands.registerCommand("nodegraph.collapseAll",()=>{P.postToActive({type:"collapseAll"})}),g.commands.registerCommand("nodegraph.expandAll",()=>{P.postToActive({type:"expandAll"})}),g.commands.registerCommand("nodegraph.new",t=>$e(t))),ue(g.workspace.workspaceFolders??[]),e.subscriptions.push(g.commands.registerCommand("nodegraph.copyAgentSpec",async t=>{let r=await ge(t);if(!r){g.window.showWarningMessage("NodeGraph: open or select a folder first \u2014 there is no workspace to copy the spec into.");return}let n=await he(e.extensionUri,r),o=await V(r),a=await fe(e.extensionUri,r);n&&o&&a?g.window.showInformationMessage(`NodeGraph: wrote .agent/NODEGRAPH_SPEC.md, .agent/ENVIRONMENT.md, and .prompt/{paper,lecture,code}/{korean,english}.md in ${r.fsPath}.`):g.window.showErrorMessage("NodeGraph: failed to write the agent files \u2014 check that the folder is writable and try again.")})),Be()}function Fe(){}0&&(module.exports={activate,deactivate});
