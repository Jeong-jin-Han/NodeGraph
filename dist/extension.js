"use strict";var ie=Object.create;var B=Object.defineProperty;var ae=Object.getOwnPropertyDescriptor;var se=Object.getOwnPropertyNames;var de=Object.getPrototypeOf,le=Object.prototype.hasOwnProperty;var ce=(e,t)=>{for(var r in t)B(e,r,{get:t[r],enumerable:!0})},z=(e,t,r,n)=>{if(t&&typeof t=="object"||typeof t=="function")for(let o of se(t))!le.call(e,o)&&o!==r&&B(e,o,{get:()=>t[o],enumerable:!(n=ae(t,o))||n.enumerable});return e};var C=(e,t,r)=>(r=e!=null?ie(de(e)):{},z(t||!e||!e.__esModule?B(r,"default",{value:e,enumerable:!0}):r,e)),pe=e=>z(B({},"__esModule",{value:!0}),e);var De={};ce(De,{activate:()=>ke,deactivate:()=>Se});module.exports=pe(De);var u=C(require("vscode"));var c=C(require("vscode")),Q=C(require("path"));var I=C(require("vscode"));function Y(e){let t=I.Uri.joinPath(e,".."),r=e.path.split("/").pop()?.replace(/\.nodegraph\.json$/,"")??"graph";return I.Uri.joinPath(t,`.${r}-imgs`)}function he(e,t,r){let n=I.Uri.joinPath(Y(t),r);return e.asWebviewUri(n).toString()}var q=/\[\[IMG:([^:\]]+)(?::[^\]]+)?\]\]/g;function j(e,t,r){let n={},o=s=>{s&&!n[s]&&(n[s]=he(e,t,s))};for(let s of r.nodes){q.lastIndex=0;let l;for(;(l=q.exec(s.content??""))!==null;)o(l[1])}for(let s of r.canvasImages??[])o(s.filename);return n}async function V(e,t,r,n="png"){let o=Y(t);try{await I.workspace.fs.createDirectory(o)}catch{}let s=`img_${Date.now()}.${n}`,l=I.Uri.joinPath(o,s);return await I.workspace.fs.writeFile(l,Buffer.from(r,"base64")),{filename:s,webviewUri:e.asWebviewUri(l).toString()}}async function K(e,t){let r=I.Uri.joinPath(Y(e),t);try{await I.workspace.fs.delete(r)}catch{}}function b(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function ue(e){let t=e.trim().replace("#",""),r=t.length===3?t.split("").map(n=>n+n).join(""):t;return/^[0-9a-fA-F]{6}$/.test(r)?{r:255-parseInt(r.slice(0,2),16),g:255-parseInt(r.slice(2,4),16),b:255-parseInt(r.slice(4,6),16)}:null}var fe=e=>e.replace(/[^a-zA-Z0-9_-]/g,"_");function $(e){return/^\s*\|/.test(e)&&e.indexOf("|",1)!==-1}function G(e){return/^\s*\|[\s\-:|]+\|\s*$/.test(e)&&!/[a-zA-Z0-9]/.test(e)}function J(e){return e.replace(/^\s*\|/,"").replace(/\|\s*$/,"").split("|").map(t=>t.trim())}function ge(e){if(!e)return[{type:"text",text:"",startChar:0,endChar:0}];let t=e.split(`
...
`),startChar:h,endChar:o})}return r}function X(e){let t=e.split(`
`);for(let r=0;r+1<t.length;r++)if($(t[r])&&G(t[r+1]))return!0;return!1}function me(e){return b(e).replace(/\\\$/g,()=>'<span class="ng-cur">$</span>')}function H(e){return me(e).replace(/\*\*(.+?)\*\*/g,'<strong style="font-size:1.1em">$1</strong>')}function L(e,t){let r=/\[\[IMG:([^:\]]+)(?::(\d+)x(\d+))?\]\]/g,n="",o=0,s;for(;(s=r.exec(e))!==null;){s.index>o&&(n+=H(e.slice(o,s.index)));let l=s[1],h=s[2],i=s[3],d=h&&i?` width="${h}" height="${i}"`:"",p=t[l];n+=p?`<img class="ng-img${d?" ng-img-sized":""}" src="${p}"${d} alt="${b(l)}" onclick="showLightbox(this.src)" title="Click to enlarge">`:`<span class="ng-img-missing">${b(l)}</span>`,o=s.index+s[0].length}return o<e.length&&(n+=H(e.slice(o))),n}function ve(e,t){let r=e.headers.map(o=>`<th>${L(o,t)}</th>`).join(""),n=e.rows.map(o=>`<tr>${o.map(s=>`<td>${L(s,t)}</td>`).join("")}</tr>`).join("");return`<div class="ng-table-wrap"><table class="ng-table"><thead><tr>${r}</tr></thead><tbody>${n}</tbody></table></div>`}function xe(e,t,r,n,o){let s=t?.color??"#888",l=t?.shape==="rounded"?"22px":"2px",h=b(t?.label??e.template),i=Math.round(e.position.x+r),d=Math.round(e.position.y+n),p="",a=e.content??"";if(X(a)){let m=ge(a);p+='<div class="ng-content">';for(let M of m)M.type==="table"?p+=ve(M,o):M.text&&(p+=`<div class="ng-seg">${L(M.text,o).replace(/\n/g,"<br>")}</div>`);p+="</div>"}else a&&(p+=`<div class="ng-content">${L(a,o).replace(/\n/g,"<br>")}</div>`);if(e.original){let m=b(e.original.title??"Original"),M=e.originalExpanded?" open":"";p+=`<details class="ng-original"${M}><summary>${m}${e.original.location?` <span class="ng-loc">${b(e.original.location)}</span>`:""}</summary>
<div class="ng-orig-text">${H(e.original.text).replace(/\n/g,"<br>")}</div></details>`}for(let m of e.toggleItems??[])p+=`<details class="ng-toggle" data-toggle-id="${b(m.id)}"${m.expanded?" open":""}><summary>${b(m.title||"(untitled)")}</summary>
<div class="ng-toggle-body">${H(m.content).replace(/\n/g,"<br>")}</div></details>`;e.links.length&&(p+=`<div class="ng-links">${e.links.map(m=>{let M=m.type==="url"?"\u{1F517}":m.type==="pdf"?"\u{1F4C4}":m.type==="obsidian"?"\u{1F7E3}":"\u2B21";return`<a class="ng-link"${m.type==="url"||m.type==="pdf"?` href="${b(m.target)}" target="_blank"`:""}>${M} ${b(m.label||m.target)}</a>`}).join("")}</div>`);let g=!!p,x=e.contentExpanded?"":' style="display:none"',k=e.children.length?` data-children="${e.children.join(",")}"`:"",S=X(a)?" ng-has-table":"",N=/\[\[IMG:[^:\]]+:(\d+)x\d+\]\]/g,E=0,A;for(;(A=N.exec(a))!==null;)E=Math.max(E,Number(A[1]));let f=E>0?X(a)?E+280:E+32:0,D=Math.max(e.nodeWidth??0,432,f),R=[D>432?`min-width:${D}px`:"",e.nodeHeight&&e.contentExpanded?`min-height:${e.nodeHeight}px`:""].filter(Boolean).join(";"),O=e.nodeHeight?` data-min-h="${e.nodeHeight}"`:"";return`<div class="ng-node${S}" id="node-${b(e.id)}"${k}${O} style="--color:${s};border-radius:${l};left:${i}px;top:${d}px${R?";"+R:""}">
  <div class="ng-header" onclick="onHeaderClick(this)" title="Click to select node">
    ...
    ${g?`<span class="ng-title" onclick="onTitleClick(event,this)" title="Click to fold/unfold">${b(e.title)}</span>`:`<span class="ng-title">${b(e.title)}</span>`}
  </div>
  ${g?`<div class="ng-body"${x}${e.fontSize?` style="font-size:${e.fontSize}px"`:""}>${p}</div>`:""}
</div>`}function Z(e,t={}){let r=1/0,n=1/0;for(let a of e.nodes)r=Math.min(r,a.position.x),n=Math.min(n,a.position.y);isFinite(r)||(r=0,n=0);let o=-r+100,s=-n+100,l=e.nodes.map(a=>xe(a,e.nodeTemplates[a.template],o,s,t)).join(`
`),h=JSON.stringify(e.nodes.map(a=>({id:a.id,lx:Math.round(a.position.x+o),ly:Math.round(a.position.y+s),children:a.children??[],template:a.template,contentExpanded:a.contentExpanded,isMain:a.template==="main_topic",nodeHeight:a.nodeHeight??null,naturalY:Math.round((a.nodeNaturalY??a.position.y)+s),title:a.title,content:a.content??"",originalTitle:a.original?.title??"",originalText:a.original?.text??"",toggles:(a.toggleItems??[]).map(g=>({id:g.id,title:g.title,content:g.content}))}))),i=JSON.stringify(e.edges.map(a=>({source:a.source,target:a.target,type:a.type,label:a.label||""}))),d=JSON.stringify(Object.fromEntries(Object.entries(e.nodeTemplates).map(([a,g])=>[a,g.label]))),p=Object.entries(e.nodeTemplates).map(([a,g])=>{let x=ue(g.color),k=x?`rgb(${x.r},${x.g},${x.b})`:"#ff3b30",S=x?`rgba(${x.r},${x.g},${x.b},0.18)`:"rgba(255,59,48,0.18)";return`::highlight(ng-hit-${fe(a)}){color:${k};background-color:${S};text-decoration:underline}`}).join(`
`);return`<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>${b(e.title)}</title>
...
<div id="toolbar">
  <div id="tb-row1">
    <span id="tb-title">${b(e.title)}</span>
  </div>
  <div id="tb-row2">
...
</html>`}var be={main_topic:{label:"Main topic",color:"#4B8BBE",icon:"file-text",shape:"sharp"},method:{label:"Method",color:"#5C9E6E",icon:"cpu",shape:"sharp"},result:{label:"Result",color:"#9B59B6",icon:"bar-chart-2",shape:"sharp"},claim:{label:"Claim",color:"#E74C3C",icon:"alert-circle",shape:"sharp"},question:{label:"Question",color:"#E5A835",icon:"help-circle",shape:"rounded"},gap:{label:"Gap / Idea",color:"#1ABC9C",icon:"lightbulb",shape:"rounded"},reference:{label:"Reference",color:"#95A5A6",icon:"book-open",shape:"rounded"},memo:{label:"Memo",color:"#BDC3C7",icon:"edit-3",shape:"rounded"}};function _(e="New Graph"){let t=new Date().toISOString();return{version:"1.0.0",title:e,created:t,modified:t,nodeTemplates:be,nodes:[],edges:[],viewport:{x:0,y:0,zoom:1}}}function F(){let e="",t="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";for(let r=0;r<32;r++)e+=t.charAt(Math.floor(Math.random()*t.length));return e}var y=C(require("vscode"));var U=class e{static{this.panels=new Map}static async openAndSearch(t,r,n,o){let s=r.toString(),l=e.panels.get(s);if(l){l.panel.reveal(y.ViewColumn.Beside,!0),l.ready?l.panel.webview.postMessage({type:"search",query:n,pageHint:o}):l.pending={query:n,pageHint:o};return}let h;try{h=await y.workspace.fs.readFile(r)}catch{y.window.showErrorMessage(`PDF\uB97C \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4: ${r.fsPath}`);return}let i=y.window.createWebviewPanel("nodegraph.pdfViewer",r.path.split("/").pop()??"PDF",{viewColumn:y.ViewColumn.Beside,preserveFocus:!1},{enableScripts:!0,retainContextWhenHidden:!0,localResourceRoots:[y.Uri.joinPath(t.extensionUri,"dist")]}),d={panel:i,ready:!1,pending:{query:n,pageHint:o}};e.panels.set(s,d),i.iconPath=y.Uri.joinPath(t.extensionUri,"resources","icon-hires.png"),i.webview.html=e._getHtml(t,i.webview);let p=Buffer.from(h).toString("base64");i.webview.onDidReceiveMessage(a=>{a.type==="ready"&&(d.ready=!0,i.webview.postMessage({type:"load",pdfData:p,query:d.pending?.query,pageHint:d.pending?.pageHint}),d.pending=null)}),i.onDidDispose(()=>{e.panels.delete(s)})}static _getHtml(t,r){let n=r.asWebviewUri(y.Uri.joinPath(t.extensionUri,"dist","pdfviewer.js")),o=r.asWebviewUri(y.Uri.joinPath(t.extensionUri,"dist","pdfviewer.css")),s=r.asWebviewUri(y.Uri.joinPath(t.extensionUri,"dist","pdf.worker.min.mjs")),l=F();return`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
</html>`}};var v=C(require("vscode")),ee=C(require("child_process"));function P(e){try{return ee.execSync(e,{timeout:5e3,stdio:["pipe","pipe","pipe"]}).toString().trim()}catch{return""}}function w(e){return P(e)!==""}function ye(){let e=[],t=new Date().toISOString(),r=process.platform,n=r==="win32"?"Windows":r==="darwin"?"macOS":"Linux",o=process.arch,s=P("python3 --version 2>&1")||P("python --version 2>&1"),l=w("python3 --version 2>&1")?"python3":w("python --version 2>&1")?"python":"",h=l!=="",i=h&&w(`${l} -c "import fitz" 2>&1 && echo ok`),d=i?P(`${l} -c "import fitz; print(fitz.__version__)"`):"",p=h&&w(`${l} -c "import pdfplumber" 2>&1 && echo ok`),a=h&&w(`${l} -c "import pdfminer" 2>&1 && echo ok`),g=h&&w(`${l} -c "from PIL import Image" 2>&1 && echo ok`),x=g?P(`${l} -c "from PIL import __version__; print(__version__)"`):"",k=h&&w(`${l} -c "import cv2" 2>&1 && echo ok`),S=w("pdftotext -v 2>&1 && echo ok")||w("pdftotext --help 2>&1 && echo ok"),N=w("convert --version 2>&1 && echo ok"),E=w("magick --version 2>&1 && echo ok"),A=w("gs --version 2>&1 && echo ok")||w("gswin64c --version 2>&1 && echo ok"),f=D=>D?"\u2705":"\u274C";return e.push("# NodeGraph \u2014 Agent Environment Report"),e.push(""),e.push("> Auto-generated by the NodeGraph extension at activation."),e.push("> **AI agents: read this file to understand what tools are available on this machine.**"),e.push("> Re-generated each time a `.nodegraph.json` file is opened."),e.push(""),e.push(`Generated: \`${t}\``),e.push(""),e.push("---"),e.push(""),e.push("## System"),e.push(""),e.push("| | |"),e.push("|---|---|"),e.push(`| OS | ${n} (\`${r}\`) |`),e.push(`| Architecture | \`${o}\` |`),e.push(`| Python | ${h?`${f(!0)} \`${s}\``:`${f(!1)} not found`} |`),e.push(`| Python command | ${h?`\`${l}\``:"N/A"} |`),e.push(""),e.push("---"),e.push(""),e.push("## PDF Reading Capabilities"),e.push(""),e.push("| Tool | Available | Notes |"),e.push("|------|:---------:|-------|"),e.push(`| PyMuPDF (\`fitz\`) | ${f(i)} | ${i?`v${d} \u2014 recommended`:"Install: `pip install pymupdf`"} |`),e.push(`| pdfplumber | ${f(p)} | ${p?"available":"Install: `pip install pdfplumber`"} |`),e.push(`| pdfminer | ${f(a)} | ${a?"available":"Install: `pip install pdfminer.six`"} |`),e.push(`| poppler (\`pdftotext\`) | ${f(S)} | ${S?"CLI tool available":r==="win32"?"Install: download poppler for Windows":r==="darwin"?"Install: `brew install poppler`":"Install: `apt install poppler-utils`"} |`),e.push(`| Ghostscript (\`gs\`) | ${f(A)} | ${A?"available":"optional"} |`),e.push(""),e.push("---"),e.push(""),e.push("## Image Processing Capabilities"),e.push(""),e.push("| Tool | Available | Notes |"),e.push("|------|:---------:|-------|"),e.push(`| Pillow (\`PIL\`) | ${f(g)} | ${g?`v${x} \u2014 recommended`:"Install: `pip install Pillow`"} |`),e.push(`| OpenCV (\`cv2\`) | ${f(k)} | ${k?"available":"Install: `pip install opencv-python`"} |`),e.push(`| ImageMagick (\`convert\`) | ${f(N||E)} | ${N||E?"CLI tool available":r==="win32"?"Install: imagemagick.org":r==="darwin"?"Install: `brew install imagemagick`":"Install: `apt install imagemagick`"} |`),e.push(""),e.push("---"),e.push(""),e.push("## Agent Recommendations"),e.push(""),h||(e.push("> \u26A0\uFE0F **Python not found.** PDF reading and image processing via Python are not available."),e.push("> Install Python from https://python.org, then reopen a `.nodegraph.json` file to re-run this check."),e.push("")),e.push("### Reading a PDF"),i?(e.push("Use PyMuPDF (recommended \u2014 fastest and most accurate):"),e.push("```python"),e.push("import fitz"),e.push('doc = fitz.open("paper.pdf")'),e.push('text = "\\n".join(page.get_text() for page in doc)'),e.push("```")):p?(e.push("Use pdfplumber:"),e.push("```python"),e.push("import pdfplumber"),e.push('with pdfplumber.open("paper.pdf") as pdf:'),e.push('    text = "\\n".join(p.extract_text() or "" for p in pdf.pages)'),e.push("```")):S?(e.push("Use poppler CLI:"),e.push("```bash"),e.push("pdftotext paper.pdf -"),e.push("```")):e.push("\u274C No PDF reading tool available. Ask the user to install PyMuPDF: `pip install pymupdf`"),e.push(""),e.push("### Extracting images from a PDF"),i?(e.push("```python"),e.push("import fitz"),e.push('doc = fitz.open("paper.pdf")'),e.push("for i, page in enumerate(doc):"),e.push("    for img in page.get_images():"),e.push("        xref = img[0]"),e.push("        pix = fitz.Pixmap(doc, xref)"),e.push('        pix.save(f"fig_{i}_{xref}.png")'),e.push("```")):g?e.push("Pillow is available but cannot extract from PDF directly. Use PyMuPDF for extraction."):e.push("\u274C No image extraction tool available."),e.push(""),e.push("---"),e.push(""),e.push("*To refresh this report, reopen any `.nodegraph.json` file.*"),e.join(`
`)}async function W(e){let t=v.Uri.joinPath(e,".agent"),r=v.Uri.joinPath(t,"ENVIRONMENT.md");try{return await v.workspace.fs.createDirectory(t),await v.workspace.fs.writeFile(r,Buffer.from(ye(),"utf-8")),!0}catch{return!1}}async function te(e){if(!(!e||e.length===0))for(let t of e)await W(t.uri)}async function re(e,t){let r=v.Uri.joinPath(e,".agent","NODEGRAPH_SPEC.md"),n;try{n=await v.workspace.fs.readFile(r)}catch{return!1}let o=v.Uri.joinPath(t,".agent"),s=v.Uri.joinPath(o,"NODEGRAPH_SPEC.md");try{return await v.workspace.fs.createDirectory(o),await v.workspace.fs.writeFile(s,n),!0}catch{return!1}}async function ne(e,t){let r=v.Uri.joinPath(e,".prompt"),n=v.Uri.joinPath(t,".prompt");try{await v.workspace.fs.createDirectory(n);for(let o of["korean.md","english.md"]){let s=await v.workspace.fs.readFile(v.Uri.joinPath(r,o));await v.workspace.fs.writeFile(v.Uri.joinPath(n,o),s)}return!0}catch{return!1}}var we=[{id:"tomoki1207.pdf",name:"vscode-pdf (PDF Viewer)"}];async function Ee(){for(let e of we)if(!u.extensions.getExtension(e.id))try{await u.commands.executeCommand("workbench.extensions.installExtension",e.id)}catch{}}async function oe(e){if(e)return e;let t=u.workspace.workspaceFolders??[];return t.length===0?void 0:t.length===1?t[0].uri:(await u.window.showWorkspaceFolderPick({placeHolder:"Select a folder for NodeGraph"}))?.uri}async function Ie(e){let t=await oe(e),r=t?u.Uri.joinPath(t,"untitled.nodegraph.json"):void 0,n=await u.window.showSaveDialog({defaultUri:r,filters:{NodeGraph:["nodegraph.json"]},title:"Create New NodeGraph"});if(!n)return;let o=n.fsPath.endsWith(".nodegraph.json")?n:n.with({path:n.path.replace(/(\.nodegraph)?(\.json)?$/,"")+".nodegraph.json"}),s=_();await u.workspace.fs.writeFile(o,Buffer.from(JSON.stringify(s,null,2),"utf-8")),await u.commands.executeCommand("vscode.openWith",o,"nodegraph.editor")}function ke(e){e.subscriptions.push(T.register(e)),e.subscriptions.push(u.commands.registerCommand("nodegraph.search",()=>{T.postToActive({type:"openSearch"})}),u.commands.registerCommand("nodegraph.fitView",()=>{T.postToActive({type:"fitView"})}),u.commands.registerCommand("nodegraph.collapseAll",()=>{T.postToActive({type:"collapseAll"})}),u.commands.registerCommand("nodegraph.expandAll",()=>{T.postToActive({type:"expandAll"})}),u.commands.registerCommand("nodegraph.new",t=>Ie(t))),te(u.workspace.workspaceFolders??[]),e.subscriptions.push(u.commands.registerCommand("nodegraph.copyAgentSpec",async t=>{let r=await oe(t);if(!r){u.window.showWarningMessage("NodeGraph: open or select a folder first \u2014 there is no workspace to copy the spec into.");return}let n=await re(e.extensionUri,r),o=await W(r),s=await ne(e.extensionUri,r);n&&o&&s?u.window.showInformationMessage(`NodeGraph: wrote .agent/NODEGRAPH_SPEC.md, .agent/ENVIRONMENT.md, and .prompt/{korean,english}.md in ${r.fsPath}.`):u.window.showErrorMessage("NodeGraph: failed to write the agent files \u2014 check that the folder is writable and try again.")})),Ee()}function Se(){}0&&(module.exports={activate,deactivate});
