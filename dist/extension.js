"use strict";var J=Object.create;var H=Object.defineProperty;var Q=Object.getOwnPropertyDescriptor;var Z=Object.getOwnPropertyNames;var ee=Object.getPrototypeOf,te=Object.prototype.hasOwnProperty;var re=(t,e)=>{for(var r in e)H(t,r,{get:e[r],enumerable:!0})},X=(t,e,r,n)=>{if(e&&typeof e=="object"||typeof e=="function")for(let a of Z(e))!te.call(t,a)&&a!==r&&H(t,a,{get:()=>e[a],enumerable:!(n=Q(e,a))||n.enumerable});return t};var M=(t,e,r)=>(r=t!=null?J(ee(t)):{},X(e||!t||!t.__esModule?H(r,"default",{value:t,enumerable:!0}):r,t)),ne=t=>X(H({},"__esModule",{value:!0}),t);var fe={};re(fe,{activate:()=>he,deactivate:()=>ge});module.exports=ne(fe);var A=M(require("vscode"));var l=M(require("vscode")),q=M(require("path"));var E=M(require("vscode"));function B(t){let e=E.Uri.joinPath(t,".."),r=t.path.split("/").pop()?.replace(/\.nodegraph\.json$/,"")??"graph";return E.Uri.joinPath(e,`.${r}-imgs`)}function oe(t,e,r){let n=E.Uri.joinPath(B(e),r);return t.asWebviewUri(n).toString()}var F=/\[\[IMG:([^:\]]+)(?::[^\]]+)?\]\]/g;function Y(t,e,r){let n={},a=s=>{s&&!n[s]&&(n[s]=oe(t,e,s))};for(let s of r.nodes){F.lastIndex=0;let h;for(;(h=F.exec(s.content??""))!==null;)a(h[1])}for(let s of r.canvasImages??[])a(s.filename);return n}async function W(t,e,r,n="png"){let a=B(e);try{await E.workspace.fs.createDirectory(a)}catch{}let s=`img_${Date.now()}.${n}`,h=E.Uri.joinPath(a,s);return await E.workspace.fs.writeFile(h,Buffer.from(r,"base64")),{filename:s,webviewUri:t.asWebviewUri(h).toString()}}async function j(t,e){let r=E.Uri.joinPath(B(t),e);try{await E.workspace.fs.delete(r)}catch{}}function m(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function ie(t){let e=t.trim().replace("#",""),r=e.length===3?e.split("").map(n=>n+n).join(""):e;return/^[0-9a-fA-F]{6}$/.test(r)?{r:255-parseInt(r.slice(0,2),16),g:255-parseInt(r.slice(2,4),16),b:255-parseInt(r.slice(4,6),16)}:null}var ae=t=>t.replace(/[^a-zA-Z0-9_-]/g,"_");function P(t){return/^\s*\|/.test(t)&&t.indexOf("|",1)!==-1}function O(t){return/^\s*\|[\s\-:|]+\|\s*$/.test(t)&&!/[a-zA-Z0-9]/.test(t)}function G(t){return t.replace(/^\s*\|/,"").replace(/\|\s*$/,"").split("|").map(e=>e.trim())}function se(t){if(!t)return[{type:"text",text:"",startChar:0,endChar:0}];let e=t.split(`
`),r=[],n=0,a=0,s=h=>e[h].length+(h<e.length-1?1:0);for(;n<e.length;)if(P(e[n])&&n+1<e.length&&O(e[n+1])){let u=a,i=[];for(;n<e.length&&P(e[n]);)i.push(e[n]),a+=s(n),n++;i.length>=3?r.push({type:"table",headers:G(i[0]),rows:i.slice(2).map(G),startChar:u,endChar:a}):r.push({type:"text",text:i.join(`
