"use strict";var ve=Object.create;var B=Object.defineProperty;var xe=Object.getOwnPropertyDescriptor;var be=Object.getOwnPropertyNames;var ye=Object.getPrototypeOf,we=Object.prototype.hasOwnProperty;var Ee=(e,t)=>{for(var r in t)B(e,r,{get:t[r],enumerable:!0})},Q=(e,t,r,n)=>{if(t&&typeof t=="object"||typeof t=="function")for(let o of be(t))!we.call(e,o)&&o!==r&&B(e,o,{get:()=>t[o],enumerable:!(n=xe(t,o))||n.enumerable});return e};var M=(e,t,r)=>(r=e!=null?ve(ye(e)):{},Q(t||!e||!e.__esModule?B(r,"default",{value:e,enumerable:!0}):r,e)),Ie=e=>Q(B({},"__esModule",{value:!0}),e);var _e={};Ee(_e,{activate:()=>Le,deactivate:()=>He});module.exports=Ie(_e);var g=M(require("vscode"));var d=M(require("vscode")),ce=M(require("path"));var T=M(require("vscode"));function X(e){let t=T.Uri.joinPath(e,".."),r=e.path.split("/").pop()?.replace(/\.nodegraph\.json$/,"")??"graph";return T.Uri.joinPath(t,`.${r}-imgs`)}function ke(e,t,r){let n=T.Uri.joinPath(X(t),r);return e.asWebviewUri(n).toString()}var ee=/\[\[IMG:([^:\]]+)(?::[^\]]+)?\]\]/g;function G(e,t,r){let n={},o=a=>{a&&!n[a]&&(n[a]=ke(e,t,a))};for(let a of r.nodes){ee.lastIndex=0;let c;for(;(c=ee.exec(a.content??""))!==null;)o(c[1])}for(let a of r.canvasImages??[])o(a.filename);return n}async function te(e,t,r,n="png"){let o=X(t);try{await T.workspace.fs.createDirectory(o)}catch{}let a=`img_${Date.now()}.${n}`,c=T.Uri.joinPath(o,a);return await T.workspace.fs.writeFile(c,Buffer.from(r,"base64")),{filename:a,webviewUri:e.asWebviewUri(c).toString()}}async function re(e,t){let r=T.Uri.joinPath(X(e),t);try{await T.workspace.fs.delete(r)}catch{}}var oe=M(require("path"));function $(e){let t=e.match(/^(.+):(\d+)(?:-(\d+))?$/);return t?{path:t[1],startLine:parseInt(t[2],10),endLine:t[3]?parseInt(t[3],10):void 0}:{path:e}}function y(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function Se(e){let t=e.trim().replace("#",""),r=t.length===3?t.split("").map(n=>n+n).join(""):t;return/^[0-9a-fA-F]{6}$/.test(r)?{r:255-parseInt(r.slice(0,2),16),g:255-parseInt(r.slice(2,4),16),b:255-parseInt(r.slice(4,6),16)}:null}var De=e=>e.replace(/[^a-zA-Z0-9_-]/g,"_");function L(e){return/^\s*\|/.test(e)&&e.indexOf("|",1)!==-1}function z(e){return/^\s*\|[\s\-:|]+\|\s*$/.test(e)&&!/[a-zA-Z0-9]/.test(e)}function ne(e){return e.replace(/^\s*\|/,"").replace(/\|\s*$/,"").split("|").map(t=>t.trim())}function Ae(e){if(!e)return[{type:"text",text:"",startChar:0,endChar:0}];let t=e.split(`
`),r=[],n=0,o=0,a=c=>t[c].length+(c<t.length-1?1:0);for(;n<t.length;)if(L(t[n])&&n+1<t.length&&z(t[n+1])){let u=o,i=[];for(;n<t.length&&L(t[n]);)i.push(t[n]),o+=a(n),n++;i.length>=3?r.push({type:"table",headers:ne(i[0]),rows:i.slice(2).map(ne),startChar:u,endChar:o}):r.push({type:"text",text:i.join(`
`),startChar:u,endChar:o})}else{let u=o,i=[];for(;n<t.length&&!(L(t[n])&&n+1<t.length&&z(t[n+1]));)i.push(t[n]),o+=a(n),n++;r.push({type:"text",text:i.join(`
`),startChar:u,endChar:o})}return r}function W(e){let t=e.split(`
`);for(let r=0;r+1<t.length;r++)if(L(t[r])&&z(t[r+1]))return!0;return!1}function Me(e){return y(e).replace(/\\\$/g,()=>'<span class="ng-cur">$</span>')}function H(e){return Me(e).replace(/\*\*(.+?)\*\*/g,'<strong style="font-size:1.1em">$1</strong>')}function _(e,t){let r=/\[\[IMG:([^:\]]+)(?::(\d+)x(\d+))?\]\]/g,n="",o=0,a;for(;(a=r.exec(e))!==null;){a.index>o&&(n+=H(e.slice(o,a.index)));let c=a[1],u=a[2],i=a[3],l=u&&i?` width="${u}" height="${i}"`:"",p=t[c];n+=p?`<img class="ng-img${l?" ng-img-sized":""}" src="${p}"${l} alt="${y(c)}" onclick="showLightbox(this.src)" title="Click to enlarge">`:`<span class="ng-img-missing">${y(c)}</span>`,o=a.index+a[0].length}return o<e.length&&(n+=H(e.slice(o))),n}function Te(e,t){let r=e.headers.map(o=>`<th>${_(o,t)}</th>`).join(""),n=e.rows.map(o=>`<tr>${o.map(a=>`<td>${_(a,t)}</td>`).join("")}</tr>`).join("");return`<div class="ng-table-wrap"><table class="ng-table"><thead><tr>${r}</tr></thead><tbody>${n}</tbody></table></div>`}function Ce(e,t,r,n,o,a){let c=t?.color??"#888",u=t?.shape==="rounded"?"22px":"2px",i=y(t?.label??e.template),l=Math.round(e.position.x+r),p=Math.round(e.position.y+n),h="",s=e.content??"";if(W(s)){let f=Ae(s);h+='<div class="ng-content">';for(let D of f)D.type==="table"?h+=Te(D,o):D.text&&(h+=`<div class="ng-seg">${_(D.text,o).replace(/\n/g,"<br>")}</div>`);h+="</div>"}else s&&(h+=`<div class="ng-content">${_(s,o).replace(/\n/g,"<br>")}</div>`);if(e.original){let f=y(e.original.title??"Original"),D=e.originalExpanded?" open":"";h+=`<details class="ng-original"${D}><summary>${f}${e.original.location?` <span class="ng-loc">${y(e.original.location)}</span>`:""}</summary>
<div class="ng-orig-text">${H(e.original.text).replace(/\n/g,"<br>")}</div></details>`}for(let f of e.toggleItems??[])h+=`<details class="ng-toggle" data-toggle-id="${y(f.id)}"${f.expanded?" open":""}><summary>${y(f.title||"(untitled)")}</summary>
<div class="ng-toggle-body">${H(f.content).replace(/\n/g,"<br>")}</div></details>`;e.links.length&&(h+=`<div class="ng-links">${e.links.map(f=>{let D=f.type==="url"?"\u{1F517}":f.type==="pdf"?"\u{1F4C4}":f.type==="obsidian"?"\u{1F7E3}":f.type==="code"?"\u{1F4BB}":"\u2B21";if(f.type==="code"){let{path:me,startLine:K,endLine:J}=$(f.target),Z=oe.posix.normalize(a.repoPrefix+me);return`<a class="ng-link"${a.githubBase&&!Z.startsWith("..")?` href="${y(a.githubBase)}/${y(Z)}${K?`#L${K}${J?`-L${J}`:""}`:""}" target="_blank"`:""}>${D} ${y(f.label||f.target)}</a>`}return`<a class="ng-link"${f.type==="url"||f.type==="pdf"?` href="${y(f.target)}" target="_blank"`:""}>${D} ${y(f.label||f.target)}</a>`}).join("")}</div>`);let b=!!h,v=e.contentExpanded?"":' style="display:none"',I=e.children.length?` data-children="${e.children.join(",")}"`:"",k=W(s)?" ng-has-table":"",C=/\[\[IMG:[^:\]]+:(\d+)x\d+\]\]/g,S=0,m;for(;(m=C.exec(s))!==null;)S=Math.max(S,Number(m[1]));let O=S>0?W(s)?S+280:S+32:0,A=Math.max(e.nodeWidth??0,432,O),N=[A>432?`min-width:${A}px`:"",e.nodeHeight&&e.contentExpanded?`min-height:${e.nodeHeight}px`:""].filter(Boolean).join(";"),Y=e.nodeHeight?` data-min-h="${e.nodeHeight}"`:"";return`<div class="ng-node${k}" id="node-${y(e.id)}"${I}${Y} style="--color:${c};border-radius:${u};left:${l}px;top:${p}px${N?";"+N:""}">
...
window.addEventListener('load', function() {
  ...
  imgs.forEach(function(img) {
    ...
    </script>`)}};var se=M(require("child_process"));function q(e,t){try{return se.execSync(e,{cwd:t,timeout:5e3,stdio:["pipe","pipe","pipe"]}).toString().trim()}catch{return""}}function Ne(e){let t=e.match(/github\.com[:/]([^/]+)\/(.+?)(?:\.git)?$/);return t?`${t[1]}/${t[2]}`:null}function de(e){let t=q("git remote get-url origin",e);if(!t)return null;let r=Ne(t);if(!r)return null;let n=q("git rev-parse HEAD",e);return n?`https://github.com/${r}/blob/${n}`:null}function le(e){let t=q("git rev-parse --show-toplevel",e);if(!t)return"";let r=e.replace(/\\/g,"/").replace(t.replace(/\\/g,"/"),"").replace(/^\/+/,"");return r?`${r}/`:""}var P=class e{constructor(t){this.context=t;this._pendingSaves=new Set}static register(t){let r=new e(t);return d.window.registerCustomEditorProvider("nodegraph.editor",r,{webviewOptions:{retainContextWhenHidden:!0}})}static{this._activeWebview=null}static postToActive(t){e._activeWebview?.postMessage(t)}async resolveCustomTextEditor(t,r,n){r.iconPath=d.Uri.joinPath(this.context.extensionUri,"resources","icon-hires.png");let o=d.Uri.joinPath(t.uri,"..");r.webview.options={enableScripts:!0,localResourceRoots:[this.context.extensionUri,o]},r.webview.html=this._getHtmlForWebview(r.webview);let a=i=>{let l=t.getText();try{let p=l.trim()===""?F():JSON.parse(l),h=G(r.webview,t.uri,p);r.webview.postMessage({type:i,data:p,imageUris:h})}catch{}},c=r.webview.onDidReceiveMessage(async i=>{if(i.type==="ready")a("load");else if(i.type==="save"){let l=t.uri.toString();this._pendingSaves.add(l);try{let p=new d.WorkspaceEdit,h=new d.Range(t.positionAt(0),t.positionAt(t.getText().length));p.replace(t.uri,h,JSON.stringify(i.data,null,2)),await d.workspace.applyEdit(p),await t.save()}finally{this._pendingSaves.delete(l)}}else if(i.type==="openLink"){let l=i.link;if(l.type==="url")d.env.openExternal(d.Uri.parse(l.target));else if(l.type==="pdf"){let p=d.Uri.joinPath(d.Uri.joinPath(t.uri,".."),l.target);d.env.openExternal(p)}else if(l.type==="obsidian")d.env.openExternal(d.Uri.parse(l.target));else if(l.type==="code"){let{path:p,startLine:h,endLine:s}=$(l.target);try{let b=d.Uri.joinPath(d.Uri.joinPath(t.uri,".."),p),v=await d.workspace.openTextDocument(b),I=await d.window.showTextDocument(v,{preview:!1});if(h){let k=Math.max(0,h-1),C=Math.max(k,(s??h)-1),S=v.lineAt(Math.min(C,v.lineCount-1)).text.length,m=new d.Range(k,0,C,S);I.selection=new d.Selection(m.start,m.end),I.revealRange(m,d.TextEditorRevealType.InCenter)}}catch{d.window.showErrorMessage(`NodeGraph: couldn't open ${p}`)}}}else if(i.type==="searchInPdf"){let l=d.Uri.joinPath(d.Uri.joinPath(t.uri,".."),i.pdfTarget);j.openAndSearch(this.context,l,i.query,i.pageHint)}else if(i.type==="exportHtml")try{let l=i.data,p=d.Uri.joinPath(t.uri,".."),h=ce.basename(t.uri.fsPath,".nodegraph.json"),s=d.Uri.joinPath(p,`.${h}-imgs`),b={},v=/\[\[IMG:([^:\]]+)(?::[^\]]+)?\]\]/g,I=async A=>{if(!(!A||b[A]))try{let N=d.Uri.joinPath(s,A),Y=await d.workspace.fs.readFile(N),f=A.split(".").pop()?.toLowerCase()??"png",D=f==="jpg"||f==="jpeg"?"image/jpeg":f==="gif"?"image/gif":f==="webp"?"image/webp":"image/png";b[A]=`data:${D};base64,${Buffer.from(Y).toString("base64")}`}catch{}};for(let A of l.nodes){v.lastIndex=0;let N;for(;(N=v.exec(A.content??""))!==null;)await I(N[1])}let k=de(p.fsPath),C=k?le(p.fsPath):"",S=ie(l,b,{githubBase:k,repoPrefix:C}),m=d.Uri.joinPath(p,`${h}.html`);await d.workspace.fs.writeFile(m,Buffer.from(S,"utf-8"));let O=await d.window.showInformationMessage(`HTML exported: ${h}.html`,"Open in Browser","Show in Explorer");O==="Open in Browser"?d.env.openExternal(m):O==="Show in Explorer"&&d.commands.executeCommand("revealFileInOS",m)}catch(l){d.window.showErrorMessage(`HTML export failed: ${l}`)}else if(i.type==="saveImage")try{let{filename:l,webviewUri:p}=await te(r.webview,t.uri,i.data,i.ext??"png");r.webview.postMessage({type:"imageSaved",nodeId:i.nodeId,filename:l,webviewUri:p})}catch(l){d.window.showErrorMessage(`Failed to save image: ${l}`)}else if(i.type==="deleteImageFile")await re(t.uri,i.filename);else if(i.type==="reload")try{let l=await d.workspace.fs.readFile(t.uri),p=Buffer.from(l).toString("utf-8"),h=JSON.parse(p),s=G(r.webview,t.uri,h);r.webview.postMessage({type:"load",data:h,imageUris:s})}catch{a("load")}else if(i.type==="openHelp"){let l=d.Uri.joinPath(this.context.extensionUri,"README.md");d.commands.executeCommand("markdown.showPreviewToSide",l.with({fragment:"features"}))}}),u=d.workspace.onDidChangeTextDocument(i=>{i.document.uri.toString()===t.uri.toString()&&(this._pendingSaves.has(t.uri.toString())||a("externalChange"))});e._activeWebview=r.webview,r.onDidChangeViewState(i=>{i.webviewPanel.active&&(e._activeWebview=r.webview,r.webview.postMessage({type:"focusCanvas"}))}),r.onDidDispose(()=>{c.dispose(),u.dispose(),e._activeWebview===r.webview&&(e._activeWebview=null)})}_getHtmlForWebview(t){let r=t.asWebviewUri(d.Uri.joinPath(this.context.extensionUri,"dist","webview.js")),n=t.asWebviewUri(d.Uri.joinPath(this.context.extensionUri,"dist","katex","katex.min.css")),o=U();return`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
